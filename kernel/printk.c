...
bool g_bootdbguart_y = false;  //flag which indicates if uart is enabled
#else
...
static size_t print_time(u64 ts, char *buf, bool with_time)
{
	...
	if (rtc_ready && !nSuspendInProgress) {
		...
		} else if (current && !with_time) {
			return sprintf(buf, "(CPU:%d-pid:%d:%s) [%02d:%02d:%02d.%09lu] ",
				this_cpu,
				current->pid,
				current->comm,
				tm.tm_hour, tm.tm_min, tm.tm_sec, timespec.tv_nsec);
		...
		if (current && with_time) {
			...
			return sprintf(buf, "(CPU:%d-pid:%d:%s) ",
				this_cpu,
				current->pid,
				current->comm);
		} else {
				...
				const char *fmt, va_list args)
{
	...
	*/
	...
	/* save timestamp ASUS style */
	ts = local_clock();
	time_size = print_time(ts, time_buf, true);

	if (is_logging_to_asus_buffer) {
		strncpy(text1, time_buf, time_size);
		strncpy(text1 + time_size, text, textlen);
		write_to_asus_log_buffer(text1, textlen + time_size, lflags);
	}

	/* append time info to the start of buffer */
	memset(time_buf, 0, 512);
	time_size = print_time(ts, time_buf, false);
	if (buflen + time_size > sizeof(buf)) {
		// skip this one, in case buffer overflow
	} else {
		memcpy(buf + buflen, time_buf, time_size);
		buflen += time_size;
	}
