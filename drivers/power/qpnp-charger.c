...
// BSP Steve2: get voltage_now and Temp workq +++
static int voltage_get_now = 0;
static int temp_get_now = 0;
// BSP Steve2: get voltage_now and Temp workq ---

// BSP Steve2: Charger limit Interface +++
...
// BSP Steve2: Demo App mode charger limit Interface +++
bool apd_charging_limit;
bool g_charging_toggle_for_apd_charging_limit;
bool demo_app_status_flag = false;
// BSP Steve2: Demo App mode charger limit Interface ---

int bms_to_get_usb_type(void)
{
	int type = 0;
...
EXPORT_SYMBOL(bms_to_get_usb_type);

static int GPIO_num69 = 69;
...
// BSP Steve2: charger done for BMS +++
int bms_to_get_chg_done(void)
{
	printk("g_chg_chip->chg_done: %d\n", g_chg_chip->chg_done);

	return g_chg_chip->chg_done;
}
EXPORT_SYMBOL(bms_to_get_chg_done);
// BSP Steve2: charger done for BMS ---
...
/*[Arima_5816][bozhi_lin] add temp_cold and temp_hot for disable charging to meet spec 20151023 begin*/
#if ( defined(CONFIG_BSP_HW_SKU_ZB551KL) || defined(CONFIG_BSP_HW_SKU_ZB501KL) )
	int vddmax_mv;
	vddmax_mv = qpnp_chg_vddmax_get(chip);
	...
	if (chip->bat_is_cold)
		...
		if (voltage_get_now >= 4100 && vddmax_mv== 4380) {
			qpnp_chg_vbatdet_set(chip, chip->cool_bat_mv
				- chip->resume_delta_mv);
		} else {
...
/*[Arima_5816][bozhi_lin] add temp_cold and temp_hot for disable charging to meet spec 20151023 begin*/
	...
	if (chip->bat_is_cold) {
		qpnp_chg_vddsafe_set(chip, chip->cool_bat_mv);
		...
		if (voltage_get_now >= 4100 && vddmax_mv== 4380) {
			...
			qpnp_chg_vddsafe_set(chip, chip->warm_bat_mv);
...
/*[Arima_5816][bozhi_lin] 20151023 end*/
		...
		qpnp_chg_vddsafe_set(chip, chip->safe_voltage_mv);
...
switch_usb_to_charge_mode(struct qpnp_chg_chip *chip)
{
	...
	gpio_direction_output(GPIO_num69, 0);  // P_+5VSO1_EN
	pr_debug("Set gpio 69 disable P_+5VSO1\n");
	schedule_delayed_work(&chip->eoc_work,
		msecs_to_jiffies(500000));

	rc = qpnp_chg_usb_suspend_enable(chip, 0);
	if (rc) {
		pr_err("%s: Set USB to suspend normal fail!\n", __FUNCTION__);
	} else {
		pr_debug("%s: Set usb to suspend normal success!\n", __FUNCTION__);
	}
...
switch_usb_to_host_mode(struct qpnp_chg_chip *chip)
{
	...
	if (rc) {
		...
		pr_debug("%s: Set usb to suspend mode success!\n", __FUNCTION__);
	}

	gpio_direction_output(GPIO_num69, 1);  // P_+5VSO1_EN
	pr_debug("---enable P_+5VSO1 GPIO69 value = %d\n", gpio_get_value(GPIO_num69));

	rc = qpnp_chg_read(chip, &usbsus, chip->usb_chgpth_base + CHGR_USB_USB_SUSP, 1);
	if (rc) {
		pr_err("Failed to read usbsus: %d\n", rc);
		return rc;
	} else {
		pr_debug("Success to read usbsus= %d\n", usbsus);
	}
...
/*[Arima_5816][bozhi_lin] fix battery capacity near 100% can't show charing icon 20151127 begin*/
	...
	if ((qpnp_chg_is_usb_chg_plugged_in(chip) ||
		qpnp_chg_is_dc_chg_plugged_in(chip))
			&& ((get_batt_capacity(chip) >= 0) && (get_batt_capacity(chip) < 100))
			&& !qpnp_chg_is_otg_en_set(chip)) {
		return POWER_SUPPLY_STATUS_CHARGING;
	}
#endif
...
{
	union power_supply_propval ret = {0,};
	int battery_status, bms_status, soc, charger_in, rc;
	...
	if (chip->bms_psy) {
		chip->bms_psy->get_property(chip->bms_psy,
				POWER_SUPPLY_PROP_CAPACITY, &ret);
		soc = ret.intval;
		rc = get_bms_calculated_soc(&batt_info.rawsoc);
		if (rc < 0) {
...
/*[Arima_5816][bozhi_lin] 20151023 end*/
				...
				&& batt_info.rawsoc <= chip->soc_resume_limit) {
			printk("resuming charging at %d%% rawsoc\n", batt_info.rawsoc);
...
qpnp_batt_external_power_changed(struct power_supply *psy)
{
	...
	if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
		...
		if ((chip->prev_usb_max_ma == ret.intval) && (get_prop_capacity(chip) != 0))
#else
...
// BSP Steve2: Demo App charger limit Interface +++
...
static bool ADF_check_status(void)
{
	...
	if (readlen != 4) {
		printk("[BAT][CHG]readlen != 4\n");
		return false;
	} else if (buf[3] == 0x1 || buf[3] == 0x2) {
		printk("[BAT][CHG]buf[3] == 0x1 || buf[3] == 0x2\n");
		return true;
	} else {
		printk("[BAT][CHG]else return false\n");
		return false;
	}
}

static int asus_apd_battery_charging_limit(void)
{
	...
	if (percentage < 0 || percentage > 100) {
		...
		if (apd_charging_limit && demo_app_status_flag) {
			if (percentage <= recharging_soc) {
				printk("%s, soc: %d <= recharging soc: %d , enable recharging\n", __FUNCTION__, percentage, recharging_soc);
				charging_status = 1;
			} else if (percentage >= charging_soc && percentage < discharging_soc) {
				printk("%s, soc: %d >= charging soc: %d and < discharging soc: %d , disable charging to battery\n", __FUNCTION__, percentage, charging_soc, discharging_soc);
				charging_status = 2;
			} else if (percentage >= discharging_soc) {
				printk("%s, soc: %d >= discharging soc: %d, discahrging status: %d\n", __FUNCTION__, percentage, discharging_soc, charging_status);
				charging_status = 3;
			}
		} else {
			printk("%s, charging limit disable, enable charging!\n", __FUNCTION__);
...
#define QPNP_CHG_ITERM_MIN_MA		80
#define QPNP_CHG_ITERM_MAX_MA		200
#define QPNP_CHG_ITERM_STEP_MA		40
...
#define QPNP_CHG_IBATMAX_1700	1700
#define QPNP_CHG_IBATMAX_2000	2000
...
qpnp_chg_set_appropriate_battery_current(struct qpnp_chg_chip *chip)
{
	...
	printk("%s , vol_get_now = %d, temp_get_now = %d, vddmax_mv = %d\n", __FUNCTION__, voltage_get_now, temp_get_now, vddmax_mv);
	if (chip->bat_is_cool)
...
qpnp_batt_system_temp_level_set(struct qpnp_chg_chip *chip, int lvl_sel)
{
	...
	printk("[BAT] %s: Thermal set to level %d\n", __FUNCTION__, lvl_sel);
	type = g_chg_chip->usb_psy->type;
	...
	if (type == POWER_SUPPLY_TYPE_USB_DCP || type == POWER_SUPPLY_TYPE_USB_CDP) {
		...
		if (lvl_sel < 2) {
			if (type == POWER_SUPPLY_TYPE_USB_DCP) {
				...
				printk("[BAT] %s: Set iusbmax to (%dmA)\n", __FUNCTION__, aicl_result);
			} else if (type == POWER_SUPPLY_TYPE_USB_CDP) {
				qpnp_chg_iusbmax_set(g_chg_chip, ASUS_CHG_I_MAX_MIN_1500);
				if (power_supply_set_current_limit(g_chg_chip->usb_psy, 1000*ASUS_CHG_I_MAX_MIN_1500))
					pr_err("%s: power supply set current limit %dmA failed\n", __FUNCTION__, ASUS_CHG_I_MAX_MIN_1500);
				/*get AICL result*/
				aicl_result = qpnp_chg_usb_iusbmax_get(g_chg_chip);
				printk("[BAT] %s: Set iusbmax to (%dmA)\n", __FUNCTION__, aicl_result);
			}
		...
		if (lvl_sel == 2) {
			...
			if (cap >= 15) {
				qpnp_chg_iusbmax_set(g_chg_chip, ASUS_CHG_I_MAX_MIN_300);
				printk("[BAT] %s: Cap = %d set ASUS_CHG_I_MAX_MIN_300 = %d\n", __FUNCTION__, cap, ASUS_CHG_I_MAX_MIN_300);
				if (power_supply_set_current_limit(g_chg_chip->usb_psy, 1000*ASUS_CHG_I_MAX_MIN_300))
					pr_err("%s: power supply set current limit %dmA failed\n", __FUNCTION__, ASUS_CHG_I_MAX_MIN_300);
			} else if (cap >= 8) {
				qpnp_chg_iusbmax_set(g_chg_chip, ASUS_CHG_I_MAX_MIN_700);
				printk("[BAT] %s: Cap = %d set ASUS_CHG_I_MAX_MIN_700 = %d\n", __FUNCTION__, cap, ASUS_CHG_I_MAX_MIN_700);
				if (power_supply_set_current_limit(g_chg_chip->usb_psy, 1000*ASUS_CHG_I_MAX_MIN_700))
					pr_err("%s: power supply set current limit %dmA failed\n", __FUNCTION__, ASUS_CHG_I_MAX_MIN_700);
			} else if (cap < 8) {
				qpnp_chg_iusbmax_set(g_chg_chip, ASUS_CHG_I_MAX_MIN_1000);
				printk("[BAT] %s: Cap = %d set ASUS_CHG_I_MAX_MIN_1000 = %d\n", __FUNCTION__, cap, ASUS_CHG_I_MAX_MIN_1000);
				if (power_supply_set_current_limit(g_chg_chip->usb_psy, 1000*ASUS_CHG_I_MAX_MIN_1000))
					pr_err("%s: power supply set current limit %dmA failed\n", __FUNCTION__, ASUS_CHG_I_MAX_MIN_1000);
			} 
			printk("[BAT] %s: lvl_sel = %d set usb enable\n", __FUNCTION__, lvl_sel);
			ret = qpnp_chg_usb_suspend_enable(g_chg_chip, 0);
			if(ret){
				pr_err("%s: Set usb to normal mode fail!\n", __FUNCTION__);
			}
		} else if (lvl_sel == 3) { /* Thermal Level = 3 */
			printk("[BAT] %s: lvl_sel = %d set usb suspend\n", __FUNCTION__, lvl_sel);
...
static int asus_print_all(void)
{
	...
	*/
	...
	snprintf(battInfo, sizeof(battInfo), "report Capacity ==>%d, FCC:%dmAh, BMS:%d, V:%dmV, Cur:%dmA, Temp:%s%d.%dC, chg_done:%d\n",
		batt_info.capacity,
		g_charge_full,
		batt_info.rawsoc,
...
{
	int ret;
	int chg_current_now, chg_current = 0;
	struct timespec tmp_time;
	int vbat_mv, temp;

	vbat_mv = get_prop_battery_voltage_now(g_chg_chip) / 1000;
	temp = get_prop_batt_temp(g_chg_chip);
	voltage_get_now = vbat_mv;
	temp_get_now = temp;

	ret = qpnp_chg_ibatmax_get(g_chg_chip, &chg_current_now);
	if (ret) {
		pr_debug("failed to get ibatmax ret=%d\n", ret);
	}
...
//BSP Steve2: Charger limit Interface +++
	#if defined(ASUS_FACTORY_BUILD)
	if (eng_charging_limit) {
		qpnp_chg_charge_en(g_chg_chip, asus_battery_charging_limit());
	} else{
		pr_err("%s: charging limit not enable!\n", __FUNCTION__);
	}
	#endif
//BSP Steve2: Charger limit Interface ---

//BSP Steve2: Demo App charger limit Interface +++
	if (apd_charging_limit) {
		if (asus_apd_battery_charging_limit() == 1) {
			qpnp_chg_usb_suspend_enable(g_chg_chip, 0);
			qpnp_chg_charge_en(g_chg_chip, 1);
		} else if (asus_apd_battery_charging_limit() == 2) {
			qpnp_chg_usb_suspend_enable(g_chg_chip, 0);
			qpnp_chg_charge_en(g_chg_chip, 0);
		} else if (asus_apd_battery_charging_limit() == 3) {
			qpnp_chg_usb_suspend_enable(g_chg_chip, 1);
			qpnp_chg_charge_en(g_chg_chip, 0);
		}
	} else{
		pr_err("%s: Demo App charging limit not enable!\n", __FUNCTION__);
	}
//BSP Steve2: Demo App charger limit Interface ---

	if (g_chg_chip->bat_is_cold || g_chg_chip->bat_is_cool || g_chg_chip->bat_is_warm || g_chg_chip->bat_is_hot) {
		qpnp_chg_set_appropriate_vddmax(g_chg_chip);
		qpnp_chg_set_appropriate_battery_current(g_chg_chip);
		qpnp_chg_set_appropriate_vbatdet(g_chg_chip);
	} else {
		if (temp_get_now >= 100 && temp_get_now < 500) {
			if (voltage_get_now <= 4000 && (temp_get_now >= 200 && temp_get_now < 500)) {
				if (chg_current_now != QPNP_CHG_IBATMAX_2000) {
					chg_current = QPNP_CHG_IBATMAX_2000;
					qpnp_chg_ibatmax_set(g_chg_chip, chg_current);
				}
			} else if ((voltage_get_now > 4000 && voltage_get_now <= 4200) && (temp_get_now >= 200 && temp_get_now < 500)) {
				if (chg_current_now != QPNP_CHG_IBATMAX_1700) {
					chg_current = QPNP_CHG_IBATMAX_1700;
					qpnp_chg_ibatmax_set(g_chg_chip, chg_current);
				}
			} else if (voltage_get_now > 4200 || (temp_get_now >= 100 && temp_get_now < 200)) {
				if (chg_current_now != g_chg_chip->cool_bat_chg_ma) {
					chg_current = g_chg_chip->cool_bat_chg_ma;
					qpnp_chg_ibatmax_set(g_chg_chip, chg_current);
				}
			}
		}
				...
				struct spmi_resource *spmi_resource)
{
	...
	case SMBCL_CHGR_SUBTYPE:
		...
		/* Steve2: CHGR VBAT TRKL 2.4V */
		rc = qpnp_chg_masked_write(chip, chip->chgr_base +
			CHGR_VBAT_TRKL,
			0xFF, 0x07, 1);

		/* Steve2: CHGR IBAT ATC_A 50mA */
		rc = qpnp_chg_masked_write(chip, chip->chgr_base +
			CHGR_IBAT_ATC_A,
			0xFF, 0x00, 1);

		/* Steve2: CHGR VBAT WEAK 3.00V */
		rc = qpnp_chg_masked_write(chip, chip->chgr_base +
			CHGR_VBAT_WEAK,
			0xFF, 0x09, 1);

		/* Steve2: CHGR IBAT ATC_B 200mA */
		rc = qpnp_chg_masked_write(chip, chip->chgr_base +
			CHGR_IBAT_ATC_B,
			0xFF, 0x02, 1);

		/* Steve2: CHGR IBAT SAFE 2000mA */
		rc = qpnp_chg_masked_write(chip, chip->chgr_base +
			CHGR_IBAT_SAFE,
			0xFF, 0x28, 1);

		/* Steve2: CHGR IBAT MAX 700mA */
		rc = qpnp_chg_masked_write(chip, chip->chgr_base +
			CHGR_IBAT_MAX,
			0xFF, 0x0E, 1);

		/* Steve2: CHGR VDD SAFE 4.38V */
		rc = qpnp_chg_masked_write(chip, chip->chgr_base +
			CHGR_VDD_SAFE,
			0xFF, 0x72, 1);

		/* Steve2: CHGR VDD MAX 4.38V */
		rc = qpnp_chg_masked_write(chip, chip->chgr_base +
			CHGR_VDD_MAX,
			0xFF, 0x72, 1);

		/* Steve2: IBAT TERM BMS 173mA */
		rc = qpnp_chg_masked_write(chip, chip->chgr_base +
			CHGR_IBAT_TERM_BMS,
			0xFF, 0x0A, 1);
	...
	case SMBCL_BAT_IF_SUBTYPE:
		...
		/* Steve2: BAT IF BTC CTRL 70%,25% */
		rc = qpnp_chg_masked_write(chip, chip->bat_if_base +
			BAT_IF_BTC_CTRL,
			0xFF, 0x80, 1);
	...
	case SMBCL_USB_CHGPTH_SUBTYPE:
		...
		/* Steve2: CHGR USB USB OTG CTL disable */
		rc = qpnp_chg_masked_write(chip,
			chip->usb_chgpth_base + CHGR_USB_USB_OTG_CTL,
			0xFF,
			0x00, 1);
	...
	case SMBBP_BOOST_SUBTYPE:
		...
		/* Steve2: BOOST ENABLE CONTROL disable */
		rc = qpnp_chg_masked_write(chip,
			chip->boost_base + BOOST_ENABLE_CONTROL,
			0xFF,
			0x00, 1);
...
static int chargerIC_status_proc_read(struct seq_file *buf, void *v)
{
	int ret = -1;
	u8 chg_sts = 0;
	ret = qpnp_chg_read(g_chg_chip, &chg_sts, INT_RT_STS(g_chg_chip->chgr_base), 1);
		...
		size_t len, loff_t *data)
{
	...
	printk("[BAT][CHG][Proc]ChargerIC Proc File: %d\n", val);
...
//BSP Steve2 proc Demo App charger_limit_enable Interface +++
static int apd_charger_limit_enable_proc_read(struct seq_file *buf, void *v)
{
	if (apd_charging_limit) {
		seq_printf(buf, "Demo App charging limit enable\n");
	} else{
		seq_printf(buf, "Demo App charging limit disable\n");
	}
	return 0;
}

static ssize_t apd_charger_limit_enable_proc_write(struct file *filp, const char __user *buff,
		size_t len, loff_t *data)
{
	char messages[256];
	...
	if (buff[0] == '1') {
		apd_charging_limit = true;
		g_charging_toggle_for_apd_charging_limit = true;
		/* turn on Demo App charging limit */
		printk("[BAT][CHG][SMB][Proc]Demo App charger_limit_enable:%d\n", 1);
	} else if (buff[0] == '0') {
		apd_charging_limit = false;
		g_charging_toggle_for_apd_charging_limit = true;
		/* turn off Demo App charging limit */
		printk("[BAT][CHG][SMB][Proc]Demo App charger_limit_enable:%d\n", 0);
	}
	return len;
}

static int apd_charger_limit_enable_proc_open(struct inode *inode, struct  file *file)
{
    return single_open(file, apd_charger_limit_enable_proc_read, NULL);
}

static const struct file_operations apd_charger_limit_enable_fops = {
	.owner = THIS_MODULE,
	.open =  apd_charger_limit_enable_proc_open,
	.write = apd_charger_limit_enable_proc_write,
	.read = seq_read,
};

static void create_apd_charger_limit_enable_proc_file(void)
{
	struct proc_dir_entry *apd_charger_limit_enable_proc_file = proc_create("driver/apd_charger_limit", 0666, NULL, &apd_charger_limit_enable_fops);

	if (apd_charger_limit_enable_proc_file) {
		printk("[BAT][CHG][SMB][Proc]Demo App charger_limit_enable create ok!\n");
	} else{
		printk("[BAT][CHG][SMB][Proc]Demo App charger_limit_enable create failed!\n");
	}
	return;
}
//BSP Steve2 proc Demo App charger_limit_enable Interface ---
...
static void batAlarm_handler(struct alarm *alarm)
{
...
/*[Arima_5816][bozhi_lin] 20151202 end*/
	...
	alarm_init(&bat_alarm, ANDROID_ALARM_RTC_WAKEUP, batAlarm_handler);
...
// BSP Steve2: Demo App charger limit Interface +++
	apd_charging_limit = false;
	g_charging_toggle_for_apd_charging_limit = true;
	create_apd_charger_limit_enable_proc_file();
// BSP Steve2: Demo App charger limit Interface ---
	...
	rc = gpio_request_one(GPIO_num69, GPIOF_OUT_INIT_LOW, "5VSUS EN");
	if (rc) {
		gpio_free(GPIO_num69);
		pr_err("Failed to request init gpio 69 Low: %d\n", rc);
	} else {
		pr_debug("Success to request init gpio 69 Low \n");
	}
	gpio_set_value(GPIO_num69,0);
	pr_debug("GPIO69 value = %d\n", gpio_get_value(GPIO_num69));
...
// BSP Steve2: Charging Thermal policy +++
	...
	printk("[BAT][CHG] %s, nextAICLinterval = %d\n", __FUNCTION__, nextAICLinterval);
