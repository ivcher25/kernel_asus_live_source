...
bool is_asus_chg_done(void)
{
	int chg_done_from_chg;
	chg_done_from_chg = bms_to_get_chg_done();
	if (chg_done_from_chg == 0) {
		printk("%s: false\n", __func__);
		return false;
	} else{
		printk("%s: true\n", __func__);
...
{
	int charge_type;
	charge_type = bms_to_get_usb_type();
	if (charge_type == -1) {
		return false;
	} else{
		return true;
...
static int init_asus_capacity(void)
{
	...
	if (fp->f_op != NULL && fp->f_op->read != NULL) {
		...
		pr_err("%s: f_op=NULL or op->read=NULL\n", __func__);
...
void backup_asus_capacity(int input)
{
	...
	if (fp->f_op != NULL && fp->f_op->write != NULL) {
		...
		pr_err("%s: f_op=NULL or op->write=NULL\n", __func__);
		return;
	}
	set_fs(old_fs);
	filp_close(fp, NULL);
...
#define ASUS_MAPPING_CHARGING_TARGET_CAPACITY	99
#define ASUS_MAPPING_CHARGING_TARGET_SHIFT	1
...
static int calculate_charging_capacity(int raw_soc, int last_rawSoc, int last_shift, int soc_check)
{
	...
	if (l_result != last_shift) {
		pr_debug("%s: raw_soc = %d, last_rawSoc = %d, last_shift = %d, soc_check = %d, result = %d\n", __func__, raw_soc, last_rawSoc, last_shift, soc_check, l_result);
	}
	return l_result;
}

#define ASUS_MAPPING_DISCHARGING_KEEP_FULL_UNTIL	98
...
static int get_asus_monotonic_soc_raw(struct qpnp_bms_chip *chip)
{
	static int l_lastRawSoc = -1;
	int raw_soc;
	int l_soc;
	raw_soc = chip->calculated_soc;
	...
	if (g_asusCapacityShift < 0) {
		...
		if (l_lastRawSoc < 0) {
			...
			pr_debug("%s: init with raw_soc = %d, shift = %d\n", __func__, raw_soc, g_asusCapacityShift);
...
static int report_cc_based_soc(struct qpnp_bms_chip *chip)
{
	...
	mutex_lock(&chip->last_soc_mutex);
	soc = get_asus_monotonic_soc_raw(chip);
					...
					int batt_temp)
{
	...
	if (params.fcc_uah - params.uuc_uah <= 0) {
		printk("FCC = %duAh, UUC = %duAh forcing soc = 0\n",
						params.fcc_uah,
						params.uuc_uah);
	...
	if (chip->first_time_calc_soc && soc != shutdown_soc
		 ...
		 * the real soc
		 */
		printk("soc = %d before forcing shutdown_soc = %d\n",
							soc, shutdown_soc);
		chip->last_ocv_uv = find_ocv_for_pc(chip, batt_temp,
	...
	if (chip->first_time_calc_soc && !chip->shutdown_soc_invalid) {
		printk("Skip adjustment when shutdown SOC has been forced\n");
...
static void charging_ended(struct qpnp_bms_chip *chip)
{
	...
	if ((get_battery_status(chip) == POWER_SUPPLY_STATUS_FULL) && (is_asus_chg_done() == true)) {
		if (chip->enable_fcc_learning &&
			(chip->start_soc <= chip->min_fcc_learning_soc) &&
			(chip->start_pc <= chip->min_fcc_ocv_pc))
			fcc_learning_config(chip, false);
		printk("chip->done_charging = true\n");
...
static void battery_status_check(struct qpnp_bms_chip *chip)
{
	...
	if (chip->battery_status != status) {
		printk("status = %d, shadow status = %d\n",
		...
		if (status == POWER_SUPPLY_STATUS_FULL) {
			...
			printk("battery not full any more\n");
			disable_bms_irq(chip, &chip->ocv_thr_irq);
			disable_bms_irq(chip, &chip->sw_cc_thr_irq);
		}
...
// BSP Steve2: edit charging_ended check flow +++
	if ((status == POWER_SUPPLY_STATUS_FULL) && (is_asus_chg_done() == true)) {
			printk("battery_status_check charging ended\n");
			charging_ended(chip);
	}
// BSP Steve2: edit charging_ended check flow ---
...
static int read_shutdown_soc(struct qpnp_bms_chip *chip)
{
	...
	printk("stored soc = 0x%02x, shutdown_soc = %d\n",
...
static void load_shutdown_data(struct qpnp_bms_chip *chip)
{
	...
	if (chip->ignore_shutdown_soc
			|| invalid_stored_soc
// BSP Steve2: remove offmode_battery_replaced check
...
static int register_spmi(struct qpnp_bms_chip *chip, struct spmi_device *spmi)
{
	...
	printk("bms-base = 0x%04x, iadc-base = 0x%04x, bat-pres-reg = 0x%04x, soc-storage-reg = 0x%04x\n",
...
/*+++BSP Steve2 Thermal Tool Interface+++*/
static int bat_testinfo_proc_read(struct seq_file *buf, void *v)
{
	int SOC, RSOC, tempr, bat_current, bat_voltage, vbat_uv, rc;
	struct qpnp_vadc_result result;
	get_battery_voltage(the_chip, &vbat_uv);
	rc = qpnp_vadc_read(the_chip->vadc_dev, LR_MUX1_BATT_THERM, &result);
	if (rc) {
		pr_err("error reading adc channel = %d, rc = %d\n",
					LR_MUX1_BATT_THERM, rc);
		return rc;
	...
	tempr = (int)result.physical;
	bat_voltage = vbat_uv / 1000;
	bat_current = get_prop_bms_current_now(the_chip) / 1000;
	SOC = get_prop_bms_capacity(the_chip);
	RSOC = the_chip->calculated_soc;
	seq_printf(buf,
		"SOC: %d\n"
		"VOLT(mv): %d\n"
...
void static create_bat_testinfo_proc_file(void)
{
	...
	if (proc_file) {
		pr_debug("[Proc]%s sucessed!\n", __FUNCTION__);
	} else{
		pr_err("[Proc]%s failed!\n", __FUNCTION__);
	}
}
...
#define BATT_VERSION "C11P1601-O-01-0003-13.14.0.57"
struct switch_dev batt_dev;
static ssize_t batt_switch_name(struct switch_dev *sdev, char *buf)
{
	char bat_version[30] = "";
	snprintf(bat_version, sizeof(bat_version), "%s", BATT_VERSION);
...
static int __devinit qpnp_bms_probe(struct spmi_device *spmi)
{
	...
	pr_info("probe success: soc = %d RSOC = %d vbatt = %d ocv = %d r_sense_uohm = %u warm_reset = %d\n",
			get_prop_bms_capacity(chip), chip->calculated_soc, vbatt, chip->last_ocv_uv,
