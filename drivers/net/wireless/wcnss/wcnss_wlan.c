...
static void wcnssctrl_rx_handler(struct work_struct *worker)
{
	...
	unsigned char fw_status = 0;
        ...
        int FW_len=0;
        int i =0;
        char FW_version[WCNSS_MAX_BUILD_VER_LEN+1];
	...
	case WCNSS_BUILD_VER_RSP:
		...
		pr_info("wcnss: build version %s\n", build);
	        ...
	        set_fs(KERNEL_DS);
                ...
                /* open file to write */
	        fp = filp_open("/asdf/driver.txt", O_WRONLY | O_CREAT | O_TRUNC, 0775);
                memset(FW_version,WCNSS_MAX_BUILD_VER_LEN+1,0);

                for(i=0;i<len;i++){
                    if(build[i]==0){
                       break;
                    }else{
                       FW_version[i] = build[i];
                       FW_len++;
                    }
                }
                if (!IS_ERR(fp)) {
                       pr_info("open file success\n");
                       fp->f_op->write(fp, FW_version, FW_len, &pos);
                       pr_info("write file success\n");
                       filp_close(fp, NULL);
                }else{
                       set_fs(old_fs);
                       pr_info("open /asdf/driver.txt fail\n");
                       break;
                }
                set_fs(old_fs);
		break;
