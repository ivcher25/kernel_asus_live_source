...
#define PS_DEFAULT_UPPER_THRESHOLD	80
...
extern void gt9xx_disable_touch(bool);
...
static int ps_mrr_table[] = { 50, 70, 100, 200, 500, 1000, 2000, 2000, 10,
				10, 10, 10, 10, 10, 10};
...
int backdoor_get_P_status(void);
...
{
	config->pinctrl = pinctrl_get(dev);
...
static int ltr553_init_device(struct ltr553_data *ltr)
{
	...
	ltr->ps_delay = 10;
	ltr->ps_measure_rate = LTR553_PS_MEASUREMENT_RATE_10MS;
	...
	/* Set proximity lower/upper threshold */
	if (ltr->ps_cal_5 == 0) {
		ltr->ps_cal_5 = PS_DEFAULT_UPPER_THRESHOLD;
		ltr->ps_cal_3 = PS_DEFAULT_UPPER_THRESHOLD + 180;
	}
	ps_data[2] = PS_LOW_BYTE(ltr->ps_cal_5);
	ps_data[3] = PS_HIGH_BYTE(ltr->ps_cal_5);
	ps_data[0] = PS_LOW_BYTE(ltr->ps_cal_3);
	ps_data[1] = PS_HIGH_BYTE(ltr->ps_cal_3);
		...
		int ch0data, int ch1data, int als_int_fac)
{
	...
	/* always trigger interrupt for calibration */
	als_data[0] = 0x0;
	als_data[1] = 0x0;
	als_data[2] = 0xff;
	als_data[3] = 0xff;
...
static int ltr553_process_data(struct ltr553_data *ltr, int als_ps)
{
	...
	if (als_ps) { /* process als data */
		...
		ltr->last_als = lux;
		ltr->last_lch0 = ch0data;
		ltr->last_lch1 = ch1data;
		...
		dev_dbg(&ltr->i2c->dev, "new als_gain:%d\n",
				ltr->als_gain);
#ifdef ASUS_FACTORY_BUILD
		pr_info("%s: lux:%d ch0data:%d ch1data:%d", __func__, lux, ch0data, ch1data);
#endif
		...
		if (tmp >= ltr->ps_cal_3) {
			...
			ps_data[2] = PS_LOW_BYTE(ltr->ps_cal_5);
			ps_data[3] = PS_HIGH_BYTE(ltr->ps_cal_5);
			ps_data[0] = PS_LOW_BYTE(0x7ff);
			ps_data[1] = PS_HIGH_BYTE(0x7ff);
			...
			ps_data[2] = PS_LOW_BYTE(0);
			ps_data[3] = PS_HIGH_BYTE(0);
			ps_data[0] = PS_LOW_BYTE(ltr->ps_cal_3);
			ps_data[1] = PS_HIGH_BYTE(ltr->ps_cal_3);
...
static int ltr553_enable_als(struct ltr553_data *ltr, int enable)
{
	...
	if (enable) {
		...
		timestamp = ktime_get_boottime();
		input_report_abs(ltr->input_light, ABS_MISC, ltr->last_als+1);
		input_sync(ltr->input_light);
		ltr->last_als_ts = timestamp;
...
static ssize_t ltr553_pdata_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned int tmp;
	int rc;
	...
	rc = regmap_bulk_read(g_ltr->regmap, LTR553_REG_PS_DATA_0,
			ps_data, 2);
	if (rc) {
		dev_err(&g_ltr->i2c->dev, "read %d failed.(%d)\n",
				LTR553_REG_PS_DATA_0, rc);
	}
...
static ssize_t ltr553_ldata_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t count = 0;
...
static ssize_t ltr553_lch0_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t count = 0;
...
static ssize_t ltr553_lch1_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t count = 0;
...
static ssize_t ltr553_ps_threshold_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct ltr553_data *ltr = dev_get_drvdata(dev);
	unsigned int up_th, low_th;
	u8 ps_data[4];
	int rc;
...
static ssize_t ltr553_ps_pulses_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct ltr553_data *ltr = dev_get_drvdata(dev);
	unsigned int pulses;
	...
	ltr->ps_pulses = pulses;

	rc = regmap_bulk_write(ltr->regmap, LTR553_REG_PS_N_PULSES,
			&pulses, 1);
	if (rc) {
		dev_err(&ltr->i2c->dev, "write %d failed.(%d)\n",
				LTR553_REG_PS_N_PULSES, rc);
		return rc;
	}
...
static ssize_t ltr553_ps_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ltr553_data *ltr = dev_get_drvdata(dev);
	ssize_t count = 0;

	count += snprintf(&buf[count], PAGE_SIZE, "3cm: %u 5cm: %u\n", ltr->ps_cal_3, ltr->ps_cal_5);

	return count;
}

static ssize_t ltr553_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t count = 0;
	count += snprintf(&buf[count], PAGE_SIZE, "1\n");
	return count;
}

static DEVICE_ATTR(register, S_IWUSR | S_IRUGO, ltr553_register_show, ltr553_register_store);
static DEVICE_ATTR(pdata, S_IRUGO, ltr553_pdata_show, NULL);
static DEVICE_ATTR(ldata, S_IRUGO, ltr553_ldata_show, NULL);
static DEVICE_ATTR(lch0, S_IRUGO, ltr553_lch0_show, NULL);
static DEVICE_ATTR(lch1, S_IRUGO, ltr553_lch1_show, NULL);
static DEVICE_ATTR(ps_threshold, S_IWUSR | S_IRUGO, ltr553_ps_threshold_show, ltr553_ps_threshold_store);
static DEVICE_ATTR(ps_pulses, S_IWUSR | S_IRUGO, NULL, ltr553_ps_pulses_store);
static DEVICE_ATTR(status, S_IRUGO, ltr553_status_show, NULL);
...
int backdoor_get_P_status(void) {
	int rc = 0;
	if (g_ltr->ps_enabled == false) {
		mutex_lock(&g_ltr->ops_lock);
		if (ltr553_enable_ps(g_ltr, 1)) {
			pr_err("%s: enable ps failed\n", __func__);
			ps_near = 0;
		}
		if (ltr553_enable_ps(g_ltr, 0)) {
			pr_err("%s: disable ps failed\n", __func__);
			ps_near = 0;
		}
		mutex_unlock(&g_ltr->ops_lock);
	} else {
		rc = ltr553_process_data(g_ltr, 0);
		if (rc) {
			dev_err(&g_ltr->i2c->dev, "process ps data failed\n");
		}
		ps_near = 0;
	}
	pr_info("%s: ps near: %d\n", __func__, ps_near);
...
EXPORT_SYMBOL(backdoor_get_P_status);
		...
		const struct i2c_device_id *id)
{
	...
	if (res) {
		dev_err(&client->dev, "initial device failed.\n");
	...
	if (gpio_is_valid(ltr->irq_gpio)) {
		...
		res = devm_request_irq(&client->dev, ltr->irq,
				ltr553_irq_handler,
				ltr->irq_flags | IRQF_ONESHOT | IRQF_TRIGGER_FALLING,
				"ltr553", ltr);
	...
	pr_info("%s: ltr553 successfully probed!", __func__);
	return 0;
...
static int __devinit elan_alsps_probe(struct platform_device *pdev)
{
	int ret = 0;
	ret = sysfs_create_file(&pdev->dev.kobj, &dev_attr_pdata.attr);
	return 0;
}

static struct platform_driver elan_alsps_driver = {
	.driver = {
		.name = "elan_alsps",
		.owner = THIS_MODULE,
	},
	.probe = elan_alsps_probe,
};

static struct platform_device elan_alsps_device = {
	.name = "elan_alsps",
	.id = -1,
};

static int __init elan_alsps_init(void)
{
	int ret = 0;
	ret = platform_driver_register(&elan_alsps_driver);
	ret = platform_device_register(&elan_alsps_device);
	return ret;
}

module_init(elan_alsps_init);
