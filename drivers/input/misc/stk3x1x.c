...
static int stk3x1x_otp_read_byte_data(struct i2c_client *client, unsigned char command)
{
	...
	if (err < 0)
		return err;
	value = err;
	pr_info("%s:read OTP 0x%x=0x%x", __func__, command, value);
...
static int stk_tune1_ct_det(struct stk3x1x_data *ps_data, uint32_t ps)
{
	...
	if (err == 0xFFFF) {
		if (ps_data->invalid_ps_cnt++ > STK_TUNE1_INVALID_PS_LIMIT) {
			pr_info("%s: invalid_ps_cnt > limit\n", __func__);
			return 3;
		}
		return 0;
	}
...
static ssize_t stk3x1x_i2c_read(struct file *filp, char __user *buffer,
	size_t count, loff_t *ppos)
{
	static int read = 0;
	if (read == 0) {
		read = 1;
		return snprintf(buffer, 5, "1\n");
	} else {
		read = 0;
		return 0;
	}
...
static const struct file_operations stk3x1x_i2c = {
	.owner= THIS_MODULE,
	.read = stk3x1x_i2c_read,
};
			 ...
			 const struct i2c_device_id *id)
{
	...
	debugfs_create_file("stk3x1x", 0644, NULL, NULL, &stk3x1x_i2c);
	ps_data->als_enabled = false;
	ps_data->ps_enabled = false;
#else
