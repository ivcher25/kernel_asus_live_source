...
#define UPDATE_FILE_PATH_1          "/asusfw/gt9xx/GT915L_Firmware.BIN" // asus_bsp ryder_hsu
#define UPDATE_FILE_PATH_2          "/sdcard/_goodix_update_.bin"

#define CONFIG_FILE_PATH_1          "/asusfw/gt9xx/GT915L_Config.cfg" // asus_bsp ryder_hsu
#define CONFIG_FILE_PATH_2          "/asusfw/gt9xx/GT915L_Config_Factory.cfg"
#define CONFIG_FILE_PATH_3			"/asusfw/gt9xx/GT915L_Config_Glove.cfg"
...
u8 got_file_flag = 0;  
u8 searching_file = 0;
extern int g_ASUS_PANEL_INFO;
extern int g_ASUS_PRJ_ID;
int glove_update_config = 0;
// asus_bsp ryder_hsu -- remove undefined variable
//extern char asus_lcd_id[2];
int config_select = 0;
#ifdef ASUS_FACTORY_BUILD
int gt915l_fw_version = 0;
#endif
...
#ifdef ASUS_FACTORY_BUILD
	gt915l_fw_version = (int)(fw_head->vid);
#endif
...
static void gup_search_file(s32 search_type)
{
    ...
    {            
        ...
        if (search_type & AUTO_SEARCH_BIN)
        {
		   ...
		   // asus_bsp ryder_hsu -- remove undefined variable
		   //GTP_INFO("asus_lcd_id[0]: %d for fw update.", asus_lcd_id[0]);
            GTP_DEBUG("Search for %s, %s for fw update.(%d/%d)", UPDATE_FILE_PATH_1, UPDATE_FILE_PATH_2, i+1, GUP_SEARCH_FILE_TIMES);
            // asus_bsp ryder_hsu -- remove undefined variable +++
			/*
			if(asus_lcd_id[0]< '3'||asus_lcd_id[0] >= '7'){
			 	pfile = filp_open("/system/etc/_goodix_update_8917.bin", O_RDONLY, 0);
            }
			else{ 
			*/
        ...
        if ( (search_type & AUTO_SEARCH_CFG) && !(got_file_flag & CFG_FILE_READY) )
        {
			...
			config_select = 1;
			#endif
			if (glove_update_config == 2)
				GTP_DEBUG("Search for %s for config update.(%d/%d)", CONFIG_FILE_PATH_3, i+1, GUP_SEARCH_FILE_TIMES);
			else
				GTP_DEBUG("Search for %s, %s for config update.(%d/%d)", CONFIG_FILE_PATH_1, CONFIG_FILE_PATH_2, i+1, GUP_SEARCH_FILE_TIMES);
            if (config_select == 0)
			{
				if (glove_update_config == 2) {
					pfile = filp_open(CONFIG_FILE_PATH_3, O_RDONLY, 0);
					if (!IS_ERR(pfile))
					{
						GTP_INFO("Cfg file: %s for config update.", CONFIG_FILE_PATH_3);
