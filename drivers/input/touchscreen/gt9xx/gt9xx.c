...
#endif
// asus_bsp ryder_hsu -- remove undefine variable
//extern char asus_lcd_id[2];
char s[20];
//asus_bsp ander add show fw_info for setting ++++
char tp_vendor_version[5];
char lcd_vendor_version[20];
u8 tp_cfg_version;
...
static s64 touch_down_timestamp_ms;
int vendor_sensor_id;
extern int glove_update_config;
extern int backdoor_get_P_status(void);
#ifdef ASUS_FACTORY_BUILD
extern int gt915l_fw_version;
...
static struct proc_dir_entry *touch_down_proc_file;
...
void gt9xx_disable_touch(bool flag)
{
	if (flag) {
		disable_tp_flag = true;
		printk("[GTP][Touch] %s: proximity trigger disable touch !\n", __func__);
	} else {
		disable_tp_flag = false;
		printk("[GTP][Touch] %s: proximity trigger enable touch  !\n", __func__);
	}
}
EXPORT_SYMBOL(gt9xx_disable_touch);

#if GTP_COMPATIBLE_FOCAL
s32 gup_i2c_read_fortest(struct i2c_client *client, u8 *buf, s32 len)
{
    struct i2c_msg msgs[2];
    s32 ret=-1;
    s32 retries = 0;
...
u8 gup_get_ic_msg_fortest(struct i2c_client *client, u16 addr, u8* msg, s32 len)

{
    ...
    msg[0] = (addr >> 8) & 0xff;
    msg[1] = addr & 0xff;
	GTP_ERROR("matt-gup_get_ic_msg_fortest");

    for (i = 0; i < 5; i++)
    {
        if (gup_i2c_read_fortest(client, msg, GTP_ADDR_LENGTH + len) > 0)
        {
            break;
        }
    }
...
{
	ktime_t touch_down_timestamp;

#if GTP_CHANGE_X2Y
    GTP_SWAP(x, y);
#endif

	touch_down_timestamp = ktime_get();
	touch_down_timestamp_ms = ktime_to_ms(touch_down_timestamp);
...
static void goodix_ts_work_func(struct work_struct *work)
{
   ...
   //GTP_DEBUG_FUNC();
    ...
    if (DOZE_ENABLED == doze_status)
    {
		ps_near = backdoor_get_P_status();
        if (ps_near){
			doze_buf[2] = 0x00;
			ret=gtp_i2c_write(i2c_connect_client, doze_buf, 3);
			gtp_enter_doze(ts);
			if (ts->use_irq)
			{
				gtp_irq_enable(ts);
	...
	{     
		   ...
		   )
		{
			...
			if (ts->gesture_mode_eable ==1){

				switch (doze_buf[2]) {

					case GESTURE_V:
						GTP_DEBUG("click GESTURE_V\n");
						if (ts->gesture_mode_type & 1 << 6){
					...
					case GESTURE_Z:
						GTP_DEBUG("click GESTURE_Z\n");
						if (ts->gesture_mode_type & 1 << 5){
					...
					case GESTURE_C:
						GTP_DEBUG("click GESTURE_C\n");
						if (ts->gesture_mode_type & 1 << 4){
						...
						break;
					case GESTURE_E:
						GTP_DEBUG("click GESTURE_E\n");
						if (ts->gesture_mode_type & 1 << 3){
					...
					case GESTURE_S:
						GTP_DEBUG("click GESTURE_S\n");

						if (ts->gesture_mode_type & 1 << 2){
					...
					case GESTURE_W:
						GTP_DEBUG("click GESTURE_W\n");
						if (ts->gesture_mode_type & 1 << 1){
			...
			{
				GTP_INFO("matt-irq run to gtp_enter_doze ");	
				// clear 0x814B
				doze_status = DOZE_WAKEUP;
				doze_buf[2] = 0x00;
				gtp_i2c_write(i2c_connect_client, doze_buf, 3);
		  ...
		  (doze_buf[2] == 0xAB) || (doze_buf[2] == 0xBA) )
		{
			if ( ts->swipeup_mode_eable ==1)
			{
				...
				GTP_INFO(" disable Swipe Up to light up the screen!");
				doze_status = DOZE_WAKEUP;
			}
			doze_buf[2] = 0x00;
			gtp_i2c_write(i2c_connect_client, doze_buf, 3);
		...
		else if (0xCC == doze_buf[2])
		{
			if ( ts->dclick_mode_eable ==1)
			{
				...
				GTP_INFO(" disable Double click to light up the screen!");
				doze_status = DOZE_WAKEUP;
			}
			doze_buf[2] = 0x00;
			ret=gtp_i2c_write(i2c_connect_client, doze_buf, 3);
			...
			*/
               ...
               if(ASUS_KEYPAD_DEBUGMODE){
                   switch(i){
                       ...
                       case 1: 
                           input_report_key(ts->input_dev, touch_key_array[3], key_value & (0x01<<i));
                           break;

                       case 2: 
                           input_report_key(ts->input_dev, touch_key_array[4], key_value & (0x01<<i));
                           break;
			...
			}else if (BACK_STATE || HOME_STATE || MENU_STATE){
				if (BACK_STATE){
					BACK_STATE = false;
...
static s8 gtp_enter_doze(struct goodix_ts_data *ts)
{
	...
	{
		GTP_DEBUG("TP has already resumed, can't go into gesture mode");
		return ret;
	}
...
*/
	tp_0_value = GTP_GPIO_GET_VALUE(gtp_tp_0);
	tp_1_value = GTP_GPIO_GET_VALUE(gtp_tp_1);
	sensor_id = tp_1_value*2 + tp_0_value;
	if (sensor_id != 0){
		GTP_ERROR("read invalid sensor_id = %d, forced to change sensor_id to 0", sensor_id);
		sensor_id = 0;
	}
    GTP_INFO("Sensor_ID: %d", sensor_id);
    vendor_sensor_id =  sensor_id;
    GTP_INFO("vendor_sensor_id: %d", vendor_sensor_id);
...
#if GTP_CUSTOM_CFG
    if(ts->lcd_vendor< '3'||ts->lcd_vendor >= '7'){
		    ...
		    config[RESOLUTION_LOC]     = (u8)1080;
                    config[RESOLUTION_LOC + 1] = (u8)(1080>>8);
                    config[RESOLUTION_LOC + 2] = (u8)1920;
                    config[RESOLUTION_LOC + 3] = (u8)(1920>>8);
     }	
...
static ssize_t gt91xx_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    ...
    {
        GTP_ERROR("size not match [%d:%d]\n", (int)GTP_CONFIG_MAX_LENGTH, (int)count);
        return -EFAULT;
    }
...
s32 gtp_read_version_forsetting(struct i2c_client *client, u16* version, char *str )
{
    s32 ret = -1;
    u8 buf[8] = {GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff};
	struct goodix_ts_data *ts = i2c_get_clientdata(client);
    ...
    {
        GTP_INFO("IC Version: %c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[7], buf[6]);
	 sprintf( str,"%c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[7], buf[6]);
    }
    else
    {
        GTP_INFO("IC Version: %c%c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
        sprintf( str,"%c%c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
    }
    //asus_bsp ander add show fw_info for setting ++++
	memset(tp_vendor_version, 0, sizeof(tp_vendor_version));
	memset(lcd_vendor_version, 0, sizeof(lcd_vendor_version));

    if (vendor_sensor_id == 0){

		sprintf(tp_vendor_version, "%s", "GIS");

	}
	else if (vendor_sensor_id==1){

		sprintf(tp_vendor_version, "%s", "BIEL");
		}
     GTP_DEBUG("Read version=%s", s);

	 switch(ts->lcd_vendor){
					case '0':
                        sprintf(lcd_vendor_version, "%s", "TM_720P");
						break;
					case '1':
						sprintf(lcd_vendor_version, "%s", "CPT5P5_720P");
						break;
					case '2':
						sprintf(lcd_vendor_version, "%s", "AUO5p5_720P");
						break;
					case '3':
						sprintf(lcd_vendor_version, "%s", "TM5P5_1080P");
						break;
					case '4':
						sprintf(lcd_vendor_version, "%s", "CTC_1080P");
						break;
					case '5':
				        sprintf(lcd_vendor_version, "%s", "AUO5p5_1080P");
						break;
					case '6':
						sprintf(lcd_vendor_version, "%s", "TM_1080P");
						break;
					case '7':
						sprintf(lcd_vendor_version, "%s", "CTC_720P");
						break;
					case '8':
						sprintf(lcd_vendor_version, "%s", "TMILI_720P");
                        break;
					case '9':
						sprintf(lcd_vendor_version, "%s", "BOEILI_720P");
						break;
					default:
						sprintf(lcd_vendor_version, "%s", "UNKOWN");
						break;
				}
	//asus_bsp ander add show fw_info for setting ---
    return ret;
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	printk("cfg_name11sid=%d\n",sid);

        if(ts->glove_mode_eable == 0)
        {

			printk("ts->glove_mode_eable=%d\n",ts->glove_mode_eable);
      	  if(ts->lcd_vendor< '3'||ts->lcd_vendor >= '7'){
			printk("ts->lcd_vendor=%d\n",ts->lcd_vendor);
		  	   GTP_ERROR("HD dts config");
			    //g= sid+8;
			   snprintf(cfg_name, sizeof(cfg_name), "goodix,cfg-group%d", sid+8);
      		}
          else {

		    printk("ts->lcd_vendor=%d\n",ts->lcd_vendor);
			GTP_ERROR("FHD dts config");
		   	snprintf(cfg_name, sizeof(cfg_name), "goodix,cfg-group%d", sid);
	        }
	       printk("cfg_name=%s\n",cfg_name);
	}
		else if (ts->glove_mode_eable == 1)
      	{
	printk("ts->glove_mode_eable1=%d\n",ts->glove_mode_eable);
	  if(ts->lcd_vendor< '3'||ts->lcd_vendor >= '7'){
			   printk("ts->lcd_vendor=%d\n",ts->lcd_vendor);
			   GTP_ERROR("HD dts1 config");
		 // g= sid+12;
				// printk("cfg_name22=%d\n",g);
			   snprintf(cfg_name, sizeof(cfg_name), "goodix,cfg-group%d", sid+12);
			     printk("cfg_name11=%s\n",cfg_name);
		}
          else {
			   ...
			   printk("ts->lcd_vendor=%d\n",ts->lcd_vendor);
			   GTP_ERROR("FHD dts config");
		       snprintf(cfg_name, sizeof(cfg_name), "goodix,cfg-group%d", sid+4);
...
{
	return scnprintf (buf, PAGE_SIZE,  "%s+IC config:%d+%s+%s\n", s,tp_cfg_version,tp_vendor_version,lcd_vendor_version) ;
}

static	ssize_t gt_touch_status_show ( struct device *dev,struct device_attribute *attr, char *buf)
{
    u16 version_info;
    s32 ret = -1;
    struct goodix_ts_data *ts = dev_get_drvdata(dev);
...
static ssize_t gt_irq_enable(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	gtp_esd_switch(ts->client, SWITCH_ON);
#endif
    printk("[Touch] gt_irq_enable ! \n");
	return nchars ;
}

static	ssize_t gt_vendor_id ( struct device *dev,struct device_attribute *attr, char *buf)
{
...
static ssize_t switch_dclick_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (tmp == 0) {
		if ((ts->swipeup_mode_eable == false) && (ts->dclick_mode_eable == true) && (ts->gesture_mode_eable == false)){
			disable_irq_wake(ts->client->irq);
		}
		ts->dclick_mode_eable = 0;
		printk("[Touch] dclick_mode_disable ! \n");

	} else if (tmp == 1) {
		if ((ts->swipeup_mode_eable == false) && (ts->dclick_mode_eable == false) && (ts->gesture_mode_eable == false)){
			enable_irq_wake(ts->client->irq);
		}
		ts->dclick_mode_eable = 1;
		printk("[Touch] dclick_mode_enable ! \n");
	}
...
static ssize_t switch_swipeup_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (tmp == 0) {
		if ((ts->swipeup_mode_eable == true) && (ts->dclick_mode_eable == false) && (ts->gesture_mode_eable == false)){
			disable_irq_wake(ts->client->irq);
		}
		ts->swipeup_mode_eable = 0;
		printk("[Touch] swipeup_mode_disable ! \n");

	} else if (tmp == 1) {
		if ((ts->swipeup_mode_eable == false) && (ts->dclick_mode_eable == false) && (ts->gesture_mode_eable == false)){
			enable_irq_wake(ts->client->irq);
		}
		ts->swipeup_mode_eable = 1;
		printk("[Touch] swipeup_mode_enable ! \n");
	}
...
static ssize_t switch_gesture_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	tmp = (ts->gesture_mode_type >> 6) & 1;

	if (!tmp)
		return sprintf(buf, "%x \n", tmp);
	else

	return sprintf(buf, "%x\n", ts->gesture_mode_type);
}
static ssize_t switch_gesture_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
		...
		printk("[Touch] gesture_mode_store %s ! \n", gesture_buf);
		if (gesture_buf[0] == cmpchar) {
		if ((ts->swipeup_mode_eable == false) && (ts->dclick_mode_eable == false) && (ts->gesture_mode_eable == false)){
			enable_irq_wake(ts->client->irq);
		}
			ts->gesture_mode_eable = 1;
			printk("[Touch] gesture_mode enable ! \n");
		} else{
			if ((ts->swipeup_mode_eable == false) && (ts->dclick_mode_eable == false) && (ts->gesture_mode_eable == true)){
				disable_irq_wake(ts->client->irq);
			}
			ts->gesture_mode_eable = 0;
		}

		if (ts->gesture_mode_eable == true) {
			...
			ts->gesture_mode_type = gesturetmp;
			printk("[Touch] gesture_mode_enable type = %x ! \n", ts->gesture_mode_type);
	
		} else {
	
			ts->gesture_mode_eable = 0;
	
	        ts->gesture_mode_type = 0;
	
			printk("[Touch] gesture_mode_disable ! \n");
			}
...
static ssize_t switch_glove_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
		...
		if (tmp == 0) {
			if (ts->glove_mode_eable == true){
				glove_update_config = 1;
				ret = gup_init_update_proc(ts);
				if (ret < 0)
				{
					GTP_ERROR("glove mode off - GTP update_proc failed.");
				}
			}
			ts->glove_mode_eable=0;
			printk("[Touch] glove mode disable");
	    } else if (tmp == 1) {
			if (ts->glove_mode_eable == false){
				glove_update_config = 2;
				ret = gup_init_update_proc(ts);
				if (ret < 0)
                {
					GTP_ERROR("glove mode on - GTP update_proc failed.");
				}
			}
			ts->glove_mode_eable=1;
			printk("[Touch] glove mode enable");
		}
...
#ifdef ASUS_FACTORY_BUILD
static ssize_t gt915l_fw_version_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", gt915l_fw_version);
}
#endif

static int touch_down_proc_read(struct seq_file *buf, void *v)
{
	seq_printf(buf, "%lld\n", touch_down_timestamp_ms);

	return 0;
}

static int touch_down_proc_open(struct inode *inode, struct  file *file)
{
    return single_open(file, touch_down_proc_read, NULL);

}

static	DEVICE_ATTR (red_hw_version, S_IRUGO, gt_hw_version_show, NULL) ;
static	DEVICE_ATTR (touch_status , S_IRUGO, gt_touch_status_show, NULL) ;
static	DEVICE_ATTR(irq_disable, S_IRUGO, gt_irq_disable, NULL);
static	DEVICE_ATTR(irq_enable, S_IRUGO, gt_irq_enable, NULL);
static	DEVICE_ATTR(vendor_id, S_IRUGO, gt_vendor_id, NULL);
...
static DEVICE_ATTR(glove_mode, S_IRUGO | S_IWUSR, switch_glove_mode_show, switch_glove_mode_store);
static	DEVICE_ATTR(swipeup_mode, S_IRUGO | S_IWUSR, switch_swipeup_mode_show, switch_swipeup_mode_store);
#ifdef ASUS_FACTORY_BUILD
static struct kobj_attribute gt915l_fw_version_attribute = {
	.attr = {.name = "vendor", .mode = 0666},
	.show = gt915l_fw_version_show,
};
static const struct attribute *gt915l_fw_version_attr[] = {
	&gt915l_fw_version_attribute.attr,
	NULL,
};
#endif
...
static const struct file_operations touch_down_proc_fops = {
	.owner = THIS_MODULE,
	.open = touch_down_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

//ASUS BSP aric: /sys/class/switch/touch ++++
static ssize_t show_tp_name(struct switch_dev *sdev, char *buf)
{
	s32 ret = -1;
	u16 version_info;
	u8 gt915_cfg_version[16] = {0};
	u16 gt915_fw_version;
	u8  opr_buf[16];

	ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA, &gt915_cfg_version[0], 1);
	if (ret != SUCCESS) {
		GTP_ERROR("Read updated IC config version failed.");
	}

	ret = gtp_i2c_read_dbl_check(ts->client, 0x8140, &opr_buf[GTP_ADDR_LENGTH], 6);
	if (ret != SUCCESS) {
		GTP_ERROR("Read updated IC fW version failed.");
	}
	gt915_fw_version = opr_buf[GTP_ADDR_LENGTH+4] + (opr_buf[GTP_ADDR_LENGTH+5]<<8);

     ret = gtp_read_version_forsetting(ts->client, &version_info, s);
    if (ret < 0)
    {
        GTP_ERROR("Read version failed.");
    }
	return scnprintf (buf, PAGE_SIZE, "%d_%d\n", gt915_fw_version, gt915_cfg_version[0]);
}
/*
static ssize_t show_tp_status(struct switch_dev *sdev, char *buf)
{
	return sprintf(buf, "touch_status=%s\n",  "1");
}
*/
//ASUS BSP aric: /sys/class/switch/touch ----
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->suspend_resume_wq = create_singlethread_workqueue("goodix_suspend_resume_wq");
	if (!ts->suspend_resume_wq) {
		  GTP_ERROR("Creat suspend_resume_wq workqueue failed.");
		return -ENOMEM;
	}
	//INIT_DELAYED_WORK(&ts->resume_work, goodix_resume_work);
	INIT_WORK(&ts->resume_work, goodix_resume_work);
    INIT_WORK(&ts->work, goodix_ts_work_func);
    ts->client = client;
	// asus_bsp ryder_hsu -- remove undefine variable
    //ts->lcd_vendor=asus_lcd_id[0];
    printk("ts->lcd_vendor=0x%x",ts->lcd_vendor);
    ...
    ret = GTP_GPIO_REQUEST(gtp_tp_0, "GTP_TP_0");
    if (ret < 0)
    {
        GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d",(s32)gtp_tp_0,ret);
		GTP_GPIO_FREE(gtp_tp_0);
        return ret;
    }
	GTP_GPIO_AS_INT(gtp_tp_0);
	ret = GTP_GPIO_REQUEST(gtp_tp_1, "GTP_TP_1");
    if (ret < 0)
    {
        GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d",(s32)gtp_tp_1,ret);
		GTP_GPIO_FREE(gtp_tp_1);
		return ret;
    }
	GTP_GPIO_AS_INT(gtp_tp_1);
#if GTP_COMPATIBLE_MODE
...
#endif
#if GTP_COMPATIBLE_FOCAL
			  ret = gup_get_ic_msg_fortest(client, 0x41E4, buf, 1);
			  if(FAIL == ret)
			  {
				  GTP_ERROR("Is focal touch.");
				   return -ENODEV;
			  }
    ...
    if (ret < 0)
    {
        GTP_ERROR("matt-I2C communication ERROR!");
    }
	...
	ts->touch_sdev.print_name = show_tp_name;
	//ts->touch_sdev.print_state = show_tp_status;

	if (switch_dev_register(&ts->touch_sdev) < 0)
	{
		printk("[TOUCH_ERR] %s: failed to register switch_dev \n", __func__);
		return -EINVAL;
	}
	//ASUS BSP aric: /sys/class/switch/touch ----
  ...
  if (ts->use_irq==0){
	...
	touch_down_proc_file = proc_create("touch_down", 0666, NULL, &touch_down_proc_fops);
	if (!touch_down_proc_file){
		GTP_ERROR("Failure %d creating proc fops\n", ret);
	}
	...
	if (!touch_kobj)
		return -ENOMEM;
	ret = sysfs_create_files(touch_kobj, gt915l_fw_version_attr);
	if (ret)
...
static int goodix_ts_remove(struct i2c_client *client)
{
	...
	remove_proc_entry("touch_down", NULL);
    if (ts) 
    {
        if (ts->use_irq)
        {
...
void goodix_ts_suspend(void)
{
	...
	if ((ts->gesture_mode_eable == true) || (ts->dclick_mode_eable == true) || (ts->swipeup_mode_eable == true)){
		ret = gtp_enter_doze(ts);
	}
	else {
		if (ts->use_irq)
...
EXPORT_SYMBOL(goodix_ts_suspend);
 ...
 void goodix_ts_resume(void)
{
	...
	GTP_INFO("touch work  before");
	queue_work(ts->suspend_resume_wq, &ts->resume_work);
  //queue_delayed_work(ts->suspend_resume_wq, &ts->resume_work,msecs_to_jiffies(200));
	GTP_INFO("touch work after ");
	return;
}
EXPORT_SYMBOL(goodix_ts_resume);
