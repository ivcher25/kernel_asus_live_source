...
#define ESD_VOLUP_KEY_DELAY_TIME 70 //ms
...
#ifdef ESD_VOLUP_KEY_DELAY_TIME
struct input_dev *volupkey_dev = 0;
static int volup_down_not_sent = 0;
static int volup_up_not_sent = 0;
static struct delayed_work __wait_for_send_volup_down_work;

void wait_for_send_volup_down_work(struct work_struct *work)
{
	if(volupkey_dev && volup_down_not_sent) {
		volup_down_not_sent = 0;
		input_event(volupkey_dev, 1, 115, 1);
		input_sync(volupkey_dev);
		printk("[KEYPAD] volup key DOWN sent! \n");
	}
}

static struct delayed_work __wait_for_send_volup_up_work;

void wait_for_send_volup_up_work(struct work_struct *work)
{
	if(volupkey_dev && volup_up_not_sent) {
		volup_up_not_sent = 0;
		input_event(volupkey_dev, 1, 115, 0);
		input_sync(volupkey_dev);
		printk("[KEYPAD] volup key UP sent! \n");
	}
}

#endif

static int volup_key_press = 0;

static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	const struct gpio_keys_button *button = bdata->button;
	struct input_dev *input = bdata->input;
#ifndef ESD_VOLUP_KEY_DELAY_TIME
	unsigned int type = button->type ?: EV_KEY;
#endif
	int state = (gpio_get_value_cansleep(button->gpio) ? 1 : 0) ^ button->active_low;
	static ktime_t  press_time,release_time;
	static s64 duration;

	if (state == 1) {
		volup_key_press = 1 ;
		press_time = ktime_get();
		printk("[KEYPAD] linux,code=%d (volume up key) , state=press \n",button->code);

		#ifdef ESD_VOLUP_KEY_DELAY_TIME
		volupkey_dev = input;
		volup_down_not_sent = 1;
		schedule_delayed_work(&__wait_for_send_volup_down_work, msecs_to_jiffies(ESD_VOLUP_KEY_DELAY_TIME));
		#endif

	} else if (state == 0 && volup_key_press == 1 ) {
		volup_key_press = 0 ;
		release_time = ktime_get();
		duration = ktime_to_ms(ktime_sub(release_time,press_time));
		printk("[KEYPAD] linux,code=%d (volume up key) , state=release ,duration time is %lld ms \n",button->code,(long long)duration);

		#ifdef ESD_VOLUP_KEY_DELAY_TIME
		volupkey_dev = input;
		if(volup_down_not_sent) {
			volup_down_not_sent = 0;
			printk("[KEYPAD] volup key DOWN not sent \n");
		} else {
			volup_up_not_sent = 1;
			schedule_delayed_work(&__wait_for_send_volup_up_work, msecs_to_jiffies(ESD_VOLUP_KEY_DELAY_TIME));
			printk("[KEYPAD] volup key DOWN sent, start sending voldown key UP \n");
		}
		#endif
	}
...
static int gpio_keys_suspend(struct device *dev)
{
	...
	if( phonestate == 1 && vol_key_wakeup == 0 ){
		...
		gpio_keys_set_key_status(dev,ddata,115,1); // vol_up key
		status_change = 1 ;
	}
		...
		gpio_keys_set_key_status(dev,ddata,115,0); // vol_up key
		status_change = 0;
	}
	//ASUS_BSP YuSiang: control volume key be wakeup source or not ---
...
#ifdef ESD_VOLUP_KEY_DELAY_TIME
	INIT_DELAYED_WORK(&__wait_for_send_volup_down_work, wait_for_send_volup_down_work);
	INIT_DELAYED_WORK(&__wait_for_send_volup_up_work, wait_for_send_volup_up_work);
#endif
