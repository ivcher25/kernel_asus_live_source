...
static void sdhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	u32 tuning_opcode;
#ifdef CONFIG_MMC_TRAP_TYPE_SDCARD_SLOT
	int cd_status = -1; // assume no card
#endif
	host = mmc_priv(mmc);
	...
	if (sdhci_check_state(host)) {
		...
		if (!(mmc->caps & MMC_CAP_NONREMOVABLE)) {
            // sdcard not eMMC
			cd_status = sdcard_tray_cd_get_status(mmc, true);
			if (cd_status != 1) {
				// cd doesn't exist, no need to resume sdcard
				printk("%s: %s: card doesn't exist(%d), not send cmd\n",__func__,mmc_hostname(mmc),cd_status);
				mrq->cmd->error = -EIO;
				if (mrq->data)
					mrq->data->error = -EIO;
