				     ...
				     size_t cnt, loff_t *ppos)
{
	...
	if(clear_user(ubuf,cnt)) {
		pr_info("%s: clear user space buffer fail!\n", __func__);
		goto exit;
	}
	temp_buf = kmalloc(CMD_STATS_TEMP_BUF_SIZE, GFP_KERNEL);
	if (!temp_buf) {
		pr_info("%s: Allocation temp kernel buffer fail!\n", __func__);
		goto exit;
	}
	to_user_buf = kmalloc(cnt, GFP_KERNEL);
	if (!to_user_buf) {
		pr_info("%s: Allocation kernel buffer fail!\n", __func__);
		goto exit;
	}
	memset(to_user_buf, 0, cnt);
	spin_lock(&card->cmd_stats->lock);


	snprintf(temp_buf, CMD_STATS_TEMP_BUF_SIZE, "%s: cmd statistics:\n",
		mmc_hostname(card->host));
	strlcat(to_user_buf, temp_buf, cnt);
	...
	pr_info("len: %d", (int)strnlen(to_user_buf, cnt));
	pr_info("%s", to_user_buf);
	if (copy_to_user(ubuf, to_user_buf, cnt)) {
		pr_info("%s: Allocation kernel buffer fail!\n", __func__);
	}
	return_size = strnlen(to_user_buf, cnt);
	kfree(temp_buf);
	kfree(to_user_buf);
