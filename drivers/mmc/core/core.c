...
#ifdef CONFIG_MMC_TRAP_TYPE_SDCARD_SLOT
// 1: card detect, 0: no card, others: error
extern int sdcard_tray_cd_get_status(struct mmc_host *host, bool log_enable);
#endif
...
int mmc_resume_bus(struct mmc_host *host)
{
	...
	if (!strcmp(mmc_hostname(host), "mmc1")) {
		// for SDCard
		cd_status = sdcard_tray_cd_get_status(host, true);
		printk("%s: %s: trying to resume sdcard!, cd_status: %d\n",__func__,mmc_hostname(host),cd_status);
		if (cd_status != 1) {
			// cd doesn't exist, no need to resume sdcard
			printk("%s: %s: card doesn't exist, cd_status: %d\n",__func__,mmc_hostname(host),cd_status);
			spin_lock_irqsave(&host->lock, flags);
		...
		printk("%s: %s: card exists, continue resume bus, cd_status %d\n",__func__,mmc_hostname(host),cd_status);
	}
#endif
...
//#ifdef CONFIG_MMC_DEBUG
#if !defined(ASUS_USER_BUILD) || defined(CONFIG_MMC_DEBUG)
...
#if CONFIG_MMC_TRAP_TYPE_SDCARD_SLOT
	cd_status = sdcard_tray_cd_get_status(host, true);
	if (cd_status == 1) {
		ret = 0; // cd still exist
		printk("%s: %s: host->ops->get_cd(): %d\n",__func__,mmc_hostname(host),cd_status);
		mmc_detect_change(host, msecs_to_jiffies(200));
	} else {
		ret = -1; // force card detection fail
		printk("%s: %s: force card remove\n",__func__,mmc_hostname(host));
...
void mmc_rescan(struct work_struct *work)
{
	...
	pr_info("%s:[SD-back] rescan_disable=%d, sd_rescan=%d\n",
		mmc_hostname(host),
...
#if CONFIG_MMC_TRAP_TYPE_SDCARD_SLOT
	if (!strcmp(mmc_hostname(host), "mmc1")) {
		cd_status = sdcard_tray_cd_get_status(host, false);
		if (cd_status == 0)
		{
			host->rescan_disable = 1; // not to rescan sdcard when no card
			mmc_claim_host(host);
			mmc_power_off(host);
			mmc_release_host(host);
		}
	...
	if (host->bus_ops != NULL) {
		pr_info("(%d) %s:[SDCard] host->bus_ops != NULL.\n",__LINE__, mmc_hostname(host));
	...
	if (host->ops->get_cd && host->ops->get_cd(host) == 0) {
		pr_info("(%d) %s:[SDCard] host->ops->get_cd && host->ops->get_cd(host).\n",__LINE__, mmc_hostname(host));
		mmc_claim_host(host);
		mmc_power_off(host);
		mmc_release_host(host);
		goto out;
...
void mmc_rpm_hold(struct mmc_host *host, struct device *dev)
{
	...
	//ASUS_BSP workaround for init SD card fail when suspend plug in SD card
	if(!strcmp(mmc_hostname(host),"mmc1"))
	{
		for (retry = 0; retry < 80; retry++) {
			...
			{
				if(retry)
					printk("%s: pm_runtime_get_sync retry(%d)\n",mmc_hostname(host),retry);
		...
		if (retry==80)
			printk("%s: pm_runtime_get_sync retry (%d) fail\n",mmc_hostname(host),retry);
	}
	else
	ret = pm_runtime_get_sync(dev);
