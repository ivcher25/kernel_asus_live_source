...
static int mmc_can_poweroff_notify(const struct mmc_card *card);	//ASUS_BSP : eMMC porting
...
static struct {
	...
	{ 0x1d5a000, "HYNIX_16G_H26M52208FPR" },
	{ 0x1d5c000, "HYNIX_16G_H26M52103FPR"},
	{ 0x1d5a000, "SANDISK_16G_SDIN9DW4" },
	{ 0x1d1f000, "SMART_16G_SE5016DMDNC1-R101T"},
};
#endif
...
static char* asus_get_emmc_status(struct mmc_card *card)
{
	...
	for (i = 0; i < EMMC_STAT_TBL_MAX; i++) {
		if (ext_csd_sector_count == emmc_stat_tbl[i].sec_cnt) {
			...
			memset(whole_name, 0, sizeof(whole_name));
			if ( ext_csd_sector_count == 0x1d5a000 && card->cid.manfid == 0x45 )
				strcpy(whole_name, "SANDISK_16G_SDIN9DW4");
			else
...
void mmc_get_manf(unsigned int id, char *manf);
static char hynix_health[128];
static char* asus_get_hynix_health(struct mmc_card *card)
{
	...
	sprintf(hynix_health , "0x%02x-%02x-%02x-%s-%s-%sG",
		card->ext_csd.device_life_time[0],
		card->ext_csd.device_life_time[1],
		card->ext_csd.device_life_time[2],
		manfname,
...
//ASUS_BSP add for DEVICE_LIFE_TIME_EST_TYP of eMMC
MMC_DEV_ATTR(emmc_health_preEOL, "0x%02x\n", card->ext_csd.device_life_time[0]);
...
//ASUS_BSP ---  "emmc info for ATD"
//ASUS_BSP add for DEVICE_LIFE_TIME_EST_TYP of eMMC
       &dev_attr_emmc_health_preEOL.attr,
...
//ASUS_BSP --- turn off PACKED CMD
	if (!oldcard) {
		...
		pr_info("[eMMC_LOG] --  Health_preEOL = 0x%02x --",card->ext_csd.device_life_time[0]);
		pr_info("[eMMC_LOG] --  Health_A      = 0x%02x --",card->ext_csd.device_life_time[1]);
		pr_info("[eMMC_LOG] --  Health_B      = 0x%02x --",card->ext_csd.device_life_time[2]);
...
static unsigned int mmc_suspend_counts = 0;
static unsigned int mmc_resume_counts = 0;
static unsigned int mmc1_suspend_counts = 0;
static unsigned int mmc1_resume_counts = 0;
#endif
...
static int mmc_suspend(struct mmc_host *host)
{
	...
	if (!strcmp(mmc_hostname(host), "mmc0")) {
		printk("%s, mmc_suspend: %u",mmc_hostname(host),mmc_suspend_counts);
		mmc_suspend_counts++;
	}
...
#ifndef ASUS_USER_BUILD
	if (!strcmp(mmc_hostname(host), "mmc0")) {
		printk("%s mmc_resume: %u",mmc_hostname(host), mmc_resume_counts);
		mmc_resume_counts++;
	}
