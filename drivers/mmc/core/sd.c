...
#include <mach/gpio.h> //ASUS_BSP: SD: add for trap only disconnect CD pin issue
#endif
...
int sdcard_tray_cd_get_status(struct mmc_host *host, bool log_enable)
{
	int ret = -ENOSYS;
	struct mmc_cd_gpio *cd = host->hotplug.handler_priv;

	if (!cd) {
		printk("%s,no cd instance\n",__func__);
		return 0;
	}

	if (!gpio_is_valid(cd->gpio)) {
		printk("%s, !gpio_is_valid(cd->gpio)\n",__func__);
	}
	...
	if (log_enable)
		printk("%s, gpio(%d), gpio_state=%d ret=%d\n",__func__,cd->gpio,gpio_get_value_cansleep(cd->gpio),ret);

out:
	return ret;
}

//EXPORT_SYMBOL(sdcard_tray_cd_get_status);

int mmc_cd_retry_get_status(struct mmc_host *host, int retry)
{
int ret = -ENOSYS;
int i = 0;

	for (i = 0; i < retry; i++) {
		ret = sdcard_tray_cd_get_status(host, true);
		if(ret == 1) {
			// has card
			return 0;
		}
...
//EXPORT_SYMBOL(mmc_cd_retry_get_status);
#endif
...
#ifdef CONFIG_MMC_TRAP_TYPE_SDCARD_SLOT
	if(sdcard_tray_cd_get_status(host, true) != 1) {
		// has no card
		err = -1;
...
static void mmc_sd_detect(struct mmc_host *host)
{
	...
	if (!retries) {
		...
		printk(KERN_ERR "%s(%s): call  _mmc_detect_card_removed(host): %d\n",
		       __func__, mmc_hostname(host), err);
	}
#else
	err = _mmc_detect_card_removed(host);
	...
	if (err) {
		printk(KERN_ERR "%s(%s): start remove card\n",__func__, mmc_hostname(host));
		mmc_sd_remove(host);
...
#if 0 //def CONFIG_MMC_TRAP_TYPE_SDCARD_SLOT
	if(sdcard_tray_cd_get_status(host, true) != 1) {
		// has no sd card
		err = -EINVAL;
		printk(KERN_ERR "%s, %s: no sdcard for attachement\n",__func__, mmc_hostname(host));
		goto err;
	}
#endif
