...
static void msm_pmic_id_status_w(struct work_struct *w)
{
	...
	dev_info(motg->phy.dev, "id_state:%d, old_id_state:%d\n", id_state, old_id_state);
...
#define ASUS_PMIC_ID_STATUS_DELAY	500 /* 500msec */
static irqreturn_t msm_pmic_id_irq(int irq, void *data)
{
	struct msm_otg *motg = data;
	int id_delay;
	__cancel_delayed_work(&motg->pmic_id_status_work);
	...
	if (fix_delay >= 0){
		id_delay = fix_delay;
		printk("Set ID de-bounce time = %d ms\n", id_delay);
	}
	else{
		id_delay = ASUS_PMIC_ID_STATUS_DELAY;
		printk("Default ID de-bounce time = %d ms\n", id_delay);
	}
