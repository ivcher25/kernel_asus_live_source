...
#define ESD_POWER_KEY_DELAY_TIME	50 //ms
#define ESD_VOLDOWN_KEY_DELAY_TIME	70 //ms
...
//+++ASUS_BSP: SW RESET SUPPORT
extern void set_dload_mode(int on);
int g_resin_enable=0;
static int set_sw_reset(char *str)
{
	g_resin_enable = 1;
	printk("SW Reset: %d\n",g_resin_enable);
	return 0;
}
__setup("resin=y", set_sw_reset);
//---ASUS_BSP: SW RESET SUPPORT
...
/* ASUS BSP: add node for factory keypad test +++ */
/* node: /sys/module/qpnp_power_on/parameters */
static int pwrkey_mode ;
unsigned int pwr_keycode;
int ASUS_KEYPAD_DEBUGMODE=0;
...
//ASUS_BSP YuSiang: control volume key irq status +++
#include <linux/msm_audio.h>
static u32 voldown_ird_id;
static int volkey_status;
int vol_down_wakeup ;
int phonecallstate = -1;
int irq_status_change = 0;

static int volkeyIRQControl_function(const char *val, struct kernel_param *kp)
{
	int ret = 0;
	int old_val = volkey_status;

	if (ret)
		return ret;

	if (volkey_status > 0xf) {
		volkey_status = old_val;
		return -EINVAL;
	}
	...
	if (volkey_status == 0) {
		vol_down_wakeup = 0;
		disable_irq_wake(voldown_ird_id);
		printk("[qpnp_pon][vol_key][irq control] disable vol_down key irq %d !\n",voldown_ird_id);

	} else if (volkey_status == 1) {
		vol_down_wakeup = 1;
		enable_irq_wake(voldown_ird_id);
		printk("[qpnp_pon][vol_key][irq control] enable vol_down key irq %d !\n",voldown_ird_id);
...
module_param_call(volkey_status, volkeyIRQControl_function, param_get_int, &volkey_status, 0644);

static int qpnp_pon_suspend(struct device *dev){

	phonecallstate= get_phonestate();
	
	/* change irq status of vol_down key when do a voice call & instacamera is off */
	if( phonecallstate == 1 && vol_down_wakeup == 0 ){
		printk("[qpnp_pon][vol_key][irq control] enable vol_down key irq, phonecallstate: %d\n",phonecallstate);
		enable_irq_wake(voldown_ird_id);
		irq_status_change = 1;
	}
	else if( phonecallstate == 0 && irq_status_change == 1 ){
		/* resume the irq status of vol_down key if it changed */
		printk("[qpnp_pon][vol_key][irq control] disable vol_down key irq, phonecallstate: %d\n",phonecallstate);
		disable_irq_wake(voldown_ird_id);
		irq_status_change = 0;
	}
...
#ifdef ESD_POWER_KEY_DELAY_TIME
struct input_dev *powerkey_dev = 0;
static int power_down_not_sent = 0;
static int power_up_not_sent = 0;
static struct delayed_work __wait_for_send_power_down_work;

void wait_for_send_power_down_work(struct work_struct *work)
{
	if(powerkey_dev && power_down_not_sent) {
		power_down_not_sent = 0;
		input_report_key(powerkey_dev, 116,1);
		input_sync(powerkey_dev);
		printk("[KEYPAD] power key DOWN sent! \n");
	}
}

static struct delayed_work __wait_for_send_power_up_work;

static void wait_for_send_power_up_work(struct work_struct *work)
{
	if(powerkey_dev && power_up_not_sent) {
		power_up_not_sent = 0;
		input_report_key(powerkey_dev, 116,0);
		input_sync(powerkey_dev);
		printk("[KEYPAD] power key UP sent! \n");
	}
}

#endif

#ifdef ESD_VOLDOWN_KEY_DELAY_TIME
struct input_dev *voldownkey_dev = 0;
static int voldown_down_not_sent = 0;
static int voldown_up_not_sent = 0;
static struct delayed_work __wait_for_send_voldown_down_work;

void wait_for_send_voldown_down_work(struct work_struct *work)
{
	if(voldownkey_dev && voldown_down_not_sent) {
		voldown_down_not_sent = 0;
		input_report_key(voldownkey_dev, 114,1);
		input_sync(voldownkey_dev);
		printk("[KEYPAD] voldown key DOWN sent! \n");
	}
}

static struct delayed_work __wait_for_send_voldown_up_work;

void wait_for_send_voldown_up_work(struct work_struct *work)
{
	if(voldownkey_dev && voldown_up_not_sent) {
		voldown_up_not_sent = 0;
		input_report_key(voldownkey_dev, 114,0);
		input_sync(voldownkey_dev);
		printk("[KEYPAD] voldown key UP sent! \n");
	}
}
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	static ktime_t  press_vol_down_key_time,release_vol_down_key_time;
	static s64 duration_vol_down_key_time;

	static ktime_t  press_power_key_time,release_power_key_time;
	static s64 duration_power_key_time;
	...
	case PON_KPDPWR:
#ifdef ASUS_FACTORY_BUILD
		/* ASUS BSP: porting factory keypad test +++ */
	...
	if ((pon_rt_sts & pon_rt_bit) > 0) { //press
		if(114 == cfg->key_code){
			press_vol_down_key_time = ktime_get();
			printk("[KEYPAD] linux,code=%d (volume down key) , state:press \n", cfg->key_code);

			#ifdef ESD_VOLDOWN_KEY_DELAY_TIME
			voldownkey_dev = pon->pon_input;
			voldown_down_not_sent = 1;
			schedule_delayed_work(&__wait_for_send_voldown_down_work, msecs_to_jiffies(ESD_VOLDOWN_KEY_DELAY_TIME));
			#endif

		}else if(116 == cfg->key_code){
			press_power_key_time = ktime_get();
			printk("[KEYPAD] linux,code=%d (power key) , state:press \n", cfg->key_code);
			#ifdef ESD_POWER_KEY_DELAY_TIME
			//cancel_delayed_work_sync(&__wait_for_send_power_up_work);
			//power_up_not_sent = 0;
			powerkey_dev = pon->pon_input;
			power_down_not_sent = 1;
			schedule_delayed_work(&__wait_for_send_power_down_work, msecs_to_jiffies(ESD_POWER_KEY_DELAY_TIME));
			#endif
		}
	} else { //release
		if(114 == cfg->key_code){
			release_vol_down_key_time = ktime_get();
			duration_vol_down_key_time = ktime_to_ms(ktime_sub(release_vol_down_key_time,press_vol_down_key_time));
			printk("[KEYPAD] linux,code=%d (volume down key) , state:release ,duration time is %lld ms \n", cfg->key_code,(long long)duration_vol_down_key_time);

			#ifdef ESD_VOLDOWN_KEY_DELAY_TIME
			voldownkey_dev = pon->pon_input;
			if(voldown_down_not_sent) {	
				voldown_down_not_sent = 0;
				printk("[KEYPAD] voldown key DOWN not sent \n");
			} else {
				voldown_up_not_sent = 1;
				schedule_delayed_work(&__wait_for_send_voldown_up_work, msecs_to_jiffies(ESD_VOLDOWN_KEY_DELAY_TIME));
				printk("[KEYPAD] voldown key DOWN sent, start sending voldown key UP \n");
			}
			#endif

		}else if(116 == cfg->key_code){
			release_power_key_time = ktime_get();
			duration_power_key_time = ktime_to_ms(ktime_sub(release_power_key_time,press_power_key_time));
			printk("[KEYPAD] linux,code=%d (power key) , state:release ,duration time is %lld ms \n", cfg->key_code,(long long)duration_power_key_time);

			#ifdef ESD_POWER_KEY_DELAY_TIME
			//cancel_delayed_work_sync(&__wait_for_send_power_down_work);
			powerkey_dev = pon->pon_input;
			if(power_down_not_sent && duration_power_key_time < 50) {
				power_down_not_sent = 0;
				printk("[KEYPAD] power key DOWN not sent \n");
			} else {
				power_up_not_sent = 1;
				schedule_delayed_work(&__wait_for_send_power_up_work, msecs_to_jiffies(ESD_POWER_KEY_DELAY_TIME));
				printk("[KEYPAD] power key DOWN sent, start sending power key UP \n");
			}
			#endif
		}
	}
	...
	while ((pp = of_get_next_child(pon->spmi->dev.of_node, pp))) {
		...
		//+++ASUS_BSP: SW RESET SUPPORT
			if(g_resin_enable) {
				rc = of_property_read_u32(pp, "qcom,support-reset",
								&cfg->support_reset);
				printk("g_resin_enable: %d, cfg->support_reset: %d\n",g_resin_enable,cfg->support_reset);
				if(cfg->support_reset) {
					printk("Set download mode for resin to ramdump!");
					set_dload_mode(1);
				}
			}
		//---ASUS_BSP: SW RESET SUPPORT
...
static int __devinit qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	// ASUS_BSP +++
	/* Get PMIC register 0xA148 */
	printk("J66: pon->spmi->sid: %d\n", pon->spmi->sid);
	if (pon->spmi->sid == 0) {
		rc = spmi_ext_register_readl(pon->spmi->ctrl, pon->spmi->sid, 0xA148, &download_mode_value, 1);
		printk("download_mode_value from 0xA148: %x\n", download_mode_value);
		if (rc) {
			download_mode_value = 0;
			dev_err(&pon->spmi->dev, "Unable to read 0xA148 register rc: %d\n", rc);
			return rc;
		}
		//reset pmic register after reading download_mode_value
		rc = spmi_ext_register_writel(pon->spmi->ctrl, pon->spmi->sid, 0xA148, &reg_reset, 1);
		if (rc) {
			dev_err(&pon->spmi->dev, "Unable to reset 0xA148 register rc: %d\n", rc);
			return rc;
		}
	}
	// ASUS_BSP ---
...
#ifdef ESD_POWER_KEY_DELAY_TIME
	INIT_DELAYED_WORK(&__wait_for_send_power_down_work, wait_for_send_power_down_work);
	INIT_DELAYED_WORK(&__wait_for_send_power_up_work, wait_for_send_power_up_work);
#endif
#ifdef ESD_VOLDOWN_KEY_DELAY_TIME
	INIT_DELAYED_WORK(&__wait_for_send_voldown_down_work, wait_for_send_voldown_down_work);
	INIT_DELAYED_WORK(&__wait_for_send_voldown_up_work, wait_for_send_voldown_up_work);
#endif
