...
#if 0 /* for factory */
#define PANEL_AVDD_GPIO 67
#define PANEL_AVEE_GPIO 119
#else
#define PANEL_AVDD_GPIO 114
#define PANEL_AVEE_GPIO 120
#endif
...
int g_charger_mode;
int g_usb_plugin = 0;
extern void as358a_resume(void);

static int mdss_dsi_ulps_config(struct mdss_dsi_ctrl_pdata *ctrl,
	int enable);

static int __init mdss_set_charger_mode_hint(char *str)
{
	if(strcmp("charger", str) == 0){
		g_charger_mode = 1;
		printk("MDSS: knowing this is charger mode\n");
	}

	return 0;
}
__setup("androidboot.mode=", mdss_set_charger_mode_hint);

static void mdss_dsi_panel_config_gate_driver_power(int enable)
{
	if (enable) {
		mdelay(15);
		gpio_tlmm_config(GPIO_CFG(PANEL_AVDD_GPIO, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		gpio_set_value(PANEL_AVDD_GPIO, enable);
		mdelay(2);
		gpio_tlmm_config(GPIO_CFG(PANEL_AVEE_GPIO, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		gpio_set_value(PANEL_AVEE_GPIO, enable);
		as358a_resume();
		mdelay(6);
	} else {
		mdelay(5);
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		/* config AVDD and AVEE external power source */
		mdss_dsi_panel_config_gate_driver_power(1);
		...
		/* config AVDD and AVEE external power source */
		mdss_dsi_panel_config_gate_driver_power(0);
	}
error:
	return ret;
}
...
static int mdss_dsi_off(struct mdss_panel_data *pdata)
{
	...
	/* config ulps */
	mdss_dsi_ulps_config(ctrl_pdata, 1);

	/* disable DSI controller */
	mdss_dsi_controller_cfg(0, pdata);

	/* disable DSI phy, comment it to have LP11 kept */
	...
	int enable)
{
	...
	int current_usb_plugin_state = g_usb_plugin;

	/* in case someone change the state concurrently */
	if (current_usb_plugin_state != g_usb_plugin) {
		current_usb_plugin_state = g_usb_plugin;
		printk("MDSS: usb plugin state mismatch.\n");
	}

	printk("MDSS: ULPS config: chg_mode(%d), usb-in(%d), ulps(%d), wfd(%d)\n",
			g_charger_mode, g_usb_plugin, ctrl->ulps, g_wfd_hint);

	/* Josh: disable ulps when wfd hint set */
	if (g_wfd_hint != 0 /*MDP_WRITEBACK_MIRROR_OFF*/)
		return 0;

	/* Josh: disable ulps when charging */
	if ((g_charger_mode || g_usb_plugin) && !ctrl->ulps)
		return 0;
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	/* disable ulps first */
	mdss_dsi_ulps_config(ctrl_pdata, 0);
 ...
 */
/*
 * LCM_ID0  TP_ID1  TP_ID0  Combo
 * --------------------------------------------
 * 0        0        0      TXD(ORISE)+Toptouch
 * 0        0        1      Unknown
 * 0        1        0      Unknown
 * 0        1        1      KD(ORISE)+Biel
 * 1        0        0      Unknown
 * 1        0        1      Tianma(ILI)+Biel
 * 1        1        0      Unknown
 * 1        1        1      KD(ORISE)+Biel
 */
void mdss_set_panel_id(void)
{
	...
	unsigned lcm_id_val = gpio_get_value(LCM_ID_PIN);
	unsigned tp0_id_val = gpio_get_value(TP_ID_PIN_0);
	unsigned tp1_id_val = gpio_get_value(TP_ID_PIN_1);

	/* checking the ID to identify panels */
	if (tp0_id_val && tp1_id_val) { // 111 & 011
		g_asus_lcm_id = 0;
	} else if (lcm_id_val && tp0_id_val && !tp1_id_val) { //110
		g_asus_lcm_id = 1;
	} else if (!lcm_id_val && !tp0_id_val && !tp1_id_val) { //000
		g_asus_lcm_id = 2;
	} else {
		g_asus_lcm_id = -1;
	}

	printk("[MDSS] read LCM ID from GPIO (%d, %d, %d): (%d, %d, %d)\n",
		LCM_ID_PIN, TP_ID_PIN_0, TP_ID_PIN_1, lcm_id_val, tp0_id_val, tp1_id_val);
}
...
static int proc_lcm_vendor_show(struct seq_file *m, void *v)
{
	...
	if (g_asus_lcm_id == 0)
		seq_printf(m, "KD LCM");
	else if (g_asus_lcm_id == 1)
		seq_printf(m, "Tianma LCM");
	else if (g_asus_lcm_id == 2)
		seq_printf(m, "TXD LCM");
	else
