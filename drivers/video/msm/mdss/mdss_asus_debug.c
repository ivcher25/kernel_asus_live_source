...
void mdss_asus_debug_cmd_write(char* cmd_buf) {
	char cmd_data[20];
...
void mdss_asus_debug_cmd_read(char* cmd_buf) {
		    ...
		    const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	//
	// project-depended command
	//
	else if (!strncmp(DSI_CMD_CUSTOM_ZB501KL_ENABLE,cmd_buf,strlen(DSI_CMD_CUSTOM_ZB501KL_ENABLE))){
		// enable custom command for otm1289a
		printk("MDSS: now enable otm1289a custom command.\n");
		strcpy(cmd_buf,"write:39 01 00 00 00 00 02 00 00");
		mdss_asus_debug_cmd_write(cmd_buf);
		strcpy(cmd_buf,"write:39 01 00 00 00 00 04 FF 12 89 01");
		mdss_asus_debug_cmd_write(cmd_buf);
		strcpy(cmd_buf,"write:39 01 00 00 00 00 02 00 80");
		mdss_asus_debug_cmd_write(cmd_buf);
		strcpy(cmd_buf,"write:39 01 00 00 00 00 03 FF 12 89");
		mdss_asus_debug_cmd_write(cmd_buf);
		goto done;
	}
	else if (!strncmp(DSI_CMD_CUSTOM_ZB501KL_DISABLE,cmd_buf,strlen(DSI_CMD_CUSTOM_ZB501KL_DISABLE))){
		// disable custom command for otm1289a
		printk("MDSS: now disable otm1289a custom command.\n");
		strcpy(cmd_buf,"write:39 01 00 00 00 00 02 00 00");
		mdss_asus_debug_cmd_write(cmd_buf);
		strcpy(cmd_buf,"write:39 01 00 00 00 00 04 FF FF FF FF");
		mdss_asus_debug_cmd_write(cmd_buf);
		goto done;
	}
	else if (!strncmp(DSI_CMD_CUSTOM_ZB501KL_P_RESET,cmd_buf,strlen(DSI_CMD_CUSTOM_ZB501KL_P_RESET))){
		// doing panel reset
		printk("MDSS: now doing panel reset.\n");
		if (!strncmp(cmd_buf+strlen(DSI_CMD_CUSTOM_ZB501KL_P_RESET), "0", 1)) {
			// disable it
			mdss_dsi_panel_reset(&(amdu_data.ctrl->panel_data), 0);
		} else {
			// enable it
			mdss_dsi_panel_reset(&(amdu_data.ctrl->panel_data), 1);
		}
		goto done;
	}
	...
	{
		mdss_asus_debug_cmd_read(cmd_buf);
	}
			...
			char __user *buff, size_t count, loff_t *ppos)
{
	...
	printk("	%s\n",STICKING_SOL1);
	tot += scnprintf(buf + tot, len - tot, "	%s\n",STICKING_SOL1);
	printk("	%s\n",DSI_CMD_CUSTOM_ZB501KL_ENABLE);
	tot += scnprintf(buf + tot, len - tot, "	%s\n",DSI_CMD_CUSTOM_ZB501KL_ENABLE);
	printk("	%s\n",DSI_CMD_CUSTOM_ZB501KL_DISABLE);
	tot += scnprintf(buf + tot, len - tot, "	%s\n",DSI_CMD_CUSTOM_ZB501KL_DISABLE);
	printk("	%s\n",DSI_CMD_CUSTOM_ZB501KL_P_RESET);
	tot += scnprintf(buf + tot, len - tot, "	%s\n",DSI_CMD_CUSTOM_ZB501KL_P_RESET);
	printk("---------------------------------------\n");
	tot += scnprintf(buf + tot, len - tot, "---------------------------------------\n");
	printk("DEBUG CMDS:\n");
