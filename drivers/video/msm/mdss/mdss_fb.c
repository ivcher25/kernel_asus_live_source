				      ...
				      enum led_brightness value)
{
	...
	if (g_asus_lcm_id == 0) {			//KD panel
		MDSS_BRIGHT_TO_BL_KD(bl_lvl, value, mfd->panel_info->bl_max,
					mfd->panel_info->brightness_max);
		if (value==0)
			bl_lvl = 0;
	} else if (g_asus_lcm_id == 1) {		//TM panel
		MDSS_BRIGHT_TO_BL_TM(bl_lvl, value, mfd->panel_info->bl_max,
					mfd->panel_info->brightness_max);
		if (value==0)
			bl_lvl = 0;
	} else {
			     ...
			     int op_enable)
{
	...
	default:
		printk("MDSS: FB_BLANK_POWERDOWN\n");
