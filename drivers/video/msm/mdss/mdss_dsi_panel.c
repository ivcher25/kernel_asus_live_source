...
static bool mdss_panel_reset_gpio_requested = false;
...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	/* send panel backlight command depends on panel vendor */
	if (g_asus_lcm_id == 1) {
		/* ZB501KL ILI9881C controller backlight shift */
		para_1 = (level & 0xF0) >> 4;
		para_2 = (level & 0x0F) << 4;
		led_pwm1[1] = (unsigned char)para_1;
		led_pwm1[2] = (unsigned char)para_2;
	} else if (g_asus_lcm_id == 0 || g_asus_lcm_id == 2) {
		/* ZB501KL OTM1289a controller has 1024 levels */
		para_1 = level / 4;
		para_2 = level % 4;
		led_pwm1[1] = (unsigned char)para_1;
		led_pwm1[2] = (unsigned char)para_2;
...
static int mdss_dsi_request_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_panel_reset_gpio_requested = true;
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (!pinfo->panel_power_on) {
			...
			printk("MDSS: doing panel reset 1.\n");
		...
		printk("MDSS: doing panel reset 0.\n");
		if (!mdss_panel_reset_gpio_requested) {
			printk("MDSS: first suspend, request reset GPIO.\n");
			mdss_dsi_request_gpios(ctrl_pdata);
		}
		gpio_direction_output((ctrl_pdata->rst_gpio), 0);
		gpio_free(ctrl_pdata->rst_gpio);
		mdss_panel_reset_gpio_requested = false;
		if (gpio_is_valid(ctrl_pdata->mode_gpio))
	...
	struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	pinfo->ulps_feature_enabled = of_property_read_bool(np,
		"qcom,ulps-enabled");
	printk("MDSS: %s: ulps feature %s", __func__,
