...
#define RF_ID_PIN0_GPIO 108
#define RF_ID_PIN1_GPIO 109
#endif
...
/*--- [5816][FM][LewisChen] Add DTV ANT Switch regulator control 20151111 Begin ---*/
struct regulator *dtv_switch = NULL;
/*--- [5816][FM][LewisChen]  20151111 End ---*/
...
/*--- [5816][FM][LewisChen] Add DTV ANT Switch regulator control 20151111 Begin ---*/
static void fm_ant_switch(int on_off);
/*--- [5816][FM][LewisChen]  20151111 End ---*/
...
static void  fm_ant_switch(int on_off)
{
	...
	if(on_off == 1) {
		if (dtv_switch == NULL) {
			dtv_switch = regulator_get(NULL, "vio_l14");
			if(IS_ERR(dtv_switch)){
				pr_err("[SPI-UT] %s: dtv_switch regulator_get failed\n",__func__);
				dtv_switch =NULL;
				return;
			}
			rc= regulator_set_voltage(dtv_switch,2750000,2750000);
			if (rc < 0) {
				pr_err("[SPI-UT] %s: regulator_set_voltage failed (%d)\n", __func__, rc);
			...
			if (rc < 0) {
				pr_err("[SPI-UT] %s: regulator_enable failed (%d)\n", __func__, rc);
				regulator_put(dtv_switch);
				dtv_switch = NULL;
				return;
			}
			pr_debug("[SPI-UT] %s:Lewis  ant_switch_power On \n",__func__);
		...
		if (dtv_switch!=NULL) {
			...
			rc = regulator_disable(dtv_switch);
			if (rc < 0) {
				pr_err("[SPI-UT] %s:reg disable(%p) failed.rc=%d\n",__func__, dtv_switch, rc);
			}
			regulator_put(dtv_switch);
			dtv_switch = NULL;
			pr_debug("[SPI-UT] %s:Lewis  ant_switch_power Off \n",__func__);
		}
	}
}
/*--- [5816][FM][LewisChen]  20151111 End ---*/
...
/*--- [5816][FM][LewisChen] Add DTV ANT Switch GPIO control 20151029 Begin ---*/
#if ( defined(CONFIG_BSP_HW_SKU_ZB551KL) || defined(CONFIG_BSP_HW_SKU_ZB501KL) )
	if( gpio_get_value(RF_ID_PIN0_GPIO) & gpio_get_value(RF_ID_PIN1_GPIO) ) {
		pr_debug("[SPI-UT] dtv_support = 1\n");
		dev_support = 1;
	}
#endif
/*--- [5816][FM][LewisChen]  20151029 End ---*/
