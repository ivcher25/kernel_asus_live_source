...
#include "msm_cci.h"
#include <linux/workqueue.h>  //ASUS_BSP: Darrency_lin ++ decrease switch time and pusn vcn to dec 0 in close stage.
#include <linux/completion.h>  //ASUS_BSP: Darrency_lin ++ decrease switch time and pusn vcn to dec 0 in close stage.
...
extern int powerup_count; //ASUS_BSP Jason fix actuator release cci
...
//ASUS_BSP: Darrency_lin ++ decrease switch time and pusn vcn to dec 0 in close stage
static void vcm_move_work(struct  work_struct *work)
{
	struct msm_actuator_ctrl_t *act_ctrl = container_of(work, struct msm_actuator_ctrl_t, work);
       uint16_t  cur_lens_pos = 0;
	uint16_t des_lens_pos = 0;
	int i=0;
	uint16_t movestep_lens = 30;
	int rc;
	u16 addr, value;

	if(act_ctrl->step_position_table==NULL){
			complete(&act_ctrl->move_done);
		}else{
			cur_lens_pos  = act_ctrl->step_position_table[act_ctrl->curr_step_pos];
			//des_lens_pos  = act_ctrl->step_position_table[0];
			des_lens_pos  = 0;//act_ctrl->step_position_table[0];
			
			for(i=cur_lens_pos - movestep_lens; i>des_lens_pos; i=i-movestep_lens)
				{
	                          addr=i>>4;
	                          value=(i &0x0f ) <<4;
	...
	//++ASUS_BSP Jason fix actuator release cci
	if (!vcm_ctrl) {
		pr_err("failed\n");
		return;
	}
	if (vcm_ctrl->act_device_type == MSM_CAMERA_PLATFORM_DEVICE) {
		//printk(" darren cci release --1 ");
		rc = vcm_ctrl->i2c_client.i2c_func_tbl->i2c_util(
			&vcm_ctrl->i2c_client, MSM_CCI_RELEASE);
		if (rc < 0)
			pr_err("MSM_CCI_RELEASE failed\n");
		}
	kfree(vcm_ctrl->i2c_reg_tbl);
	vcm_ctrl->i2c_reg_tbl = NULL;
	//--ASUS_BSP Jason fix actuator release cci
}
 //ASUS_BSP: Darrency_lin -- decrease switch time and pusn vcn to dec 0 in close stage
...
//ASUS_BSP: Darrency_lin ++ decrease switch time and pusn vcn to dec 0 in close stage
void  msm_actuator_close_move(void)
{
	int16_t rc=0;
	CDBG("Enter\n");
	init_completion(&vcm_ctrl->move_done);
	schedule_work(&vcm_ctrl->work);
	rc = wait_for_completion_timeout(
			&vcm_ctrl->move_done, msecs_to_jiffies(800));
...
EXPORT_SYMBOL(msm_actuator_close_move);
...
//ASUS_BSP: Darrency_lin ++ decrease switch time and pusn vcn to dec 0 in close stage
	INIT_WORK(&msm_actuator_t->work, vcm_move_work);
//ASUS_BSP: Darrency_lin --  decrease switch time and pusn vcn to dec 0 in close stage
