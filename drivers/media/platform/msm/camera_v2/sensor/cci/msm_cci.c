...
//ASUS BSP Bryant +++ Avoid CCI timeout in open camera state.
//#define CCI_TIMEOUT msecs_to_jiffies(100)
#define CCI_TIMEOUT msecs_to_jiffies(150)
//ASUS BSP Bryant --- Avoid CCI timeout in open camera state.
	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
	...
	enum cci_i2c_master_t master;
	cci_dev = v4l2_get_subdevdata(sd);
// printk("%s %d jason_i=%d\n",__func__,__LINE__,jason_i);
	if (!cci_dev || !c_ctrl) {
...
static int32_t msm_cci_release(struct v4l2_subdev *sd)
{
	...
	if (cci_dev->cci_state != CCI_STATE_ENABLED) {
		pr_err("%s invalid ref count %d / cci state CCI_STATE_ENABLED %d\n",
			__func__, cci_dev->ref_count, cci_dev->cci_state);
//		return -EINVAL;
	}
	if (!cci_dev->ref_count) {
		pr_err("%s invalid ref count %d / cci state %d    ref_count =0\n",
			__func__, cci_dev->ref_count, cci_dev->cci_state);
//		return -EINVAL;
	}

	if (--cci_dev->ref_count) {
		if(cci_dev->ref_count ==255) cci_dev->ref_count=0;
		printk("%s ref_count Exit %d   line=%d \n", __func__, cci_dev->ref_count,__LINE__);
		return 0;
	}
		printk("%s ref_count  %d line=%d\n", __func__, cci_dev->ref_count,__LINE__);

// Tingyi: Don't disable IRQ or device will random reboot. Don't why...
//	disable_irq(cci_dev->irq->start);
