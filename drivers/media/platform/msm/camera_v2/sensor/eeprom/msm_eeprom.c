...
int Main_Camera_Lens_ID = 0;//[5816][Camera]Add for Judge JVIE different Lens Person Liu 20160330
...
/*void re_read_eeprom_data(struct msm_eeprom_ctrl_t *e_ctrl,
				       struct msm_eeprom_cfg_data *cdata)
{
...
/*        if(e_ctrl->readdata_again)  //darren 0327
        	re_read_eeprom_data(e_ctrl,cdata);*/
...
bool  check_eeprom_checksum(struct msm_eeprom_ctrl_t *e_ctrl, int index)
{
	  uint8_t data_in[186];
  	  uint8_t lenc_out[360];
	  uint32_t checksumLSC = 0, checksumOTP = 0, checksumTotal = 0;
         uint32_t checksumLSC2 = 0, checksumOTP2 = 0, checksumTotal2 = 0;
	  int i, offset;
	  bool checkresult;
	...
	offset = 17+index*189+1;
	
        for(i=0;i<186;i++) {
	       checksumLSC += (uint8_t)(e_ctrl->cal_data.mapdata[offset + i]);
        }
	  for(i = 0; i < sizeof(data_in); i++){
	       data_in[i] = (uint8_t)(e_ctrl->cal_data.mapdata[offset + i]);
       }
  ...
  checksumLSC2 = e_ctrl->cal_data.mapdata[offset + 186];
  checksumOTP2 = e_ctrl->cal_data.mapdata[offset + 187];
  checksumTotal2 = e_ctrl->cal_data.mapdata[offset + 188];

  printk(" %s  LSC = 0x%x, 0x%x,; OTP =  0x%x, 0x%x; SUM =  0x%x, 0x%x",__func__, checksumLSC,checksumLSC2,checksumOTP,checksumOTP2,checksumTotal,checksumTotal2);

  if(checksumLSC == checksumLSC2 && checksumOTP== checksumOTP2){
	checkresult=true;
  }
  else if(checksumTotal == checksumTotal2){
	checkresult=true;
  }else
     checkresult=false;
...
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	if (rc < 0)
		goto board_free;
if(strcmp(eb_info->eeprom_name,"ov5695_otp"))
{
do{
	rc = msm_camera_power_up(power_info, e_ctrl->eeprom_device_type,
		&e_ctrl->i2c_client);
...
//[5816][Camera]Add for Judge JVIE different Lens Person Liu 20160330 S
	if(rc == 0){
		if (((e_ctrl->cal_data.mapdata[0] >> 6) & 0x03) == 1){
			Main_13M_Camera_version = e_ctrl->cal_data.mapdata[1];
			Main_Camera_Lens_ID = e_ctrl->cal_data.mapdata[2];
			pr_err("Main_13M_Camera group 1, Data: 0x%X",e_ctrl->cal_data.mapdata[0]);
			//pr_err("  lsc1_s=0x%x otp_s=0x%x tot_s=0x%x",e_ctrl->cal_data.mapdata[204] ,e_ctrl->cal_data.mapdata[205] 
				//,e_ctrl->cal_data.mapdata[206] );
			result = check_eeprom_checksum(e_ctrl, 0);
		}
		else{
			Main_13M_Camera_version = e_ctrl->cal_data.mapdata[9];
			Main_Camera_Lens_ID = e_ctrl->cal_data.mapdata[10];			
			pr_err("Main_13M_Camera group 2, Data: 0x%X",e_ctrl->cal_data.mapdata[0]);
			//pr_err("  lsc2_s=0x%x otp_s=0x%x tot_s=0x%x",e_ctrl->cal_data.mapdata[393] ,e_ctrl->cal_data.mapdata[394] 
				//,e_ctrl->cal_data.mapdata[395] );
                    	result = check_eeprom_checksum(e_ctrl, 1);
		}
		pr_err("Main_13M_Camera_version = 0x%X  Lens_ID = 0x%X\n",Main_13M_Camera_version,Main_Camera_Lens_ID);
	}
	pr_err(" %s  read eeprom data times = %d ",__func__, retrytimes+1);
	if(result!=true){
		pr_info(" <<<<<<< ov13850  checksum failure  >>>>>>>>>");
		retrytimes++;
		}else{
		pr_info(" <<<<<<< ov13850  checksum success  >>>>>>>>>");
		}
	...
	if(rc == 0){
		if (((e_ctrl->cal_data.mapdata[0] >> 6) & 0x03) == 1){
			...
			pr_err("Main_8M_Camera group 1, Data: 0x%X",e_ctrl->cal_data.mapdata[0]);
		}
		else{
			Main_8M_Camera_version = e_ctrl->cal_data.mapdata[9];
			pr_err("Main_8M_Camera group 2, Data: 0x%X",e_ctrl->cal_data.mapdata[0]);
		}
		pr_err("Main_8M_Camera_version = 0x%X\n",Main_8M_Camera_version);
	}
//[5816][Camera]Add for Judge JVIE different Lens Person Liu 20160330 E
...
//[5816][camera] Add for camera 2nd sourece 20151201 Person E
    ...
    }while(result==false && retrytimes<2);
}	
