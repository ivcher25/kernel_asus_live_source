...
#include <linux/time.h> //ASUS_BSP: Darrency_lin ++ decrease switch time
...
//ASUS_BSP Stimber_Hsueh "Implement for ATD interface" +++
unsigned char g_camera_status1[4] = {1, 1, 1,1};
...
static void create_readotp_proc_file(void);
static void create_printotp_proc_file(void);
//ASUS_BSP darrency_lin --
...
extern int Main_Camera_Lens_ID;//[5816][Camera]Add for Judge JVIE different Lens Person Liu 20160330
...
int powerdown_done = 0;
//ASUS_BSP : darrency_lin ++
static char   OTPDATA[30];
char  ALLOTPDATA[30];
bool isreb=false;
//ASUS_BSP : darrency_lin --

static void sensor_powerdown_work(struct  work_struct *work)
{
	struct msm_sensor_ctrl_t *s_ctrl = container_of(work, struct msm_sensor_ctrl_t, work);
	...
	sensor_i2c_client = s_ctrl->sensor_i2c_client;
	 ...
	 msm_camera_power_down(power_info, sensor_device_type,
		sensor_i2c_client);
	 powerdown_done = 1;
}
//ASUS_BSP: Darrency_lin -- decrease switch time
...
//ASUS_BSP Evan : Modify the moving steps for actuator +++
/*	if(isPowerup){ //ASUS_BSP: Darrency_lin -- decrease switch time
		actuator_close();
	}*/ //darren
//ASUS_BSP Evan : Modify the moving steps for actuator ---
	power_info = &s_ctrl->sensordata->power_info;
	sensor_device_type = s_ctrl->sensor_device_type;
	sensor_i2c_client = s_ctrl->sensor_i2c_client;
	//module_name = s_ctrl->sensordata->sensor_name;
...
//ASUS_BSP Evan : Modify the moving steps for actuator +++
	isPowerup=0;
//ASUS_BSP Evan : Modify the moving steps for actuator ---
	schedule_work(&s_ctrl->work); //ASUS_BSP: Darrency_lin -- decrease switch time

sensor_i2c_client->sensorname = s_ctrl->sensordata->sensor_name;
return 0;
...
int msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	module_name = s_ctrl->sensordata->sensor_name; //darren 
...
//ASUS_BSP: Darrency_lin -- decrease switch time
	...
	if (rc < 0){
               ...
               printk("%s msm_sensor_match_id first fail \n",__func__);
               ...
               rc = msm_sensor_check_id(s_ctrl);
               if(rc <0){
                       printk("%s msm_sensor_match_id second fail \n",__func__);
                       msm_camera_power_down(power_info, s_ctrl->sensor_device_type,sensor_i2c_client);
                       rc = msm_camera_power_up(power_info, s_ctrl->sensor_device_type,sensor_i2c_client);
                       if (rc < 0)
                               return rc;
                       rc = msm_sensor_check_id(s_ctrl);
                       if(rc <0){
                               printk("%s msm_sensor_match_id third fail \n",__func__);
                               msm_camera_power_down(power_info, s_ctrl->sensor_device_type,sensor_i2c_client);
			}
		}
	}
...
void read_allotp_data(struct msm_camera_i2c_client *sensor_i2c_client)
{
	int err;
 	uint32_t addr=0;
	uint16_t valgroup1[15];
	
    	sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0x0100 , 0x01  ,MSM_CAMERA_I2C_BYTE_ADDR);
	sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0x3d84 , 0xc0  ,MSM_CAMERA_I2C_BYTE_ADDR);
	sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0x3d88 , 0x70  ,MSM_CAMERA_I2C_BYTE_ADDR);
	sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0x3d89 , 0x0c ,MSM_CAMERA_I2C_BYTE_ADDR);
	sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0x3d8a , 0x70  ,MSM_CAMERA_I2C_BYTE_ADDR);
	sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0x3d8b , 0x1b  ,MSM_CAMERA_I2C_BYTE_ADDR);
	sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0x3d81 , 0x00  ,MSM_CAMERA_I2C_BYTE_ADDR);
	sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0x3d81 , 0x01  ,MSM_CAMERA_I2C_BYTE_ADDR);
	mdelay(1);

			         for(addr=0x700c;addr<0x701b;addr++){
					err=sensor_i2c_client->i2c_func_tbl->i2c_read(sensor_i2c_client,addr,&valgroup1[addr-0x700c],
						MSM_CAMERA_I2C_BYTE_ADDR);
					      // printk("====== [0x%x]=0x%x ======= \r\n",addr-0x700c, valgroup1[addr-0x700c]);
					msleep(2);
		         	}
				sprintf(ALLOTPDATA,"%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x",valgroup1[0],valgroup1[1],valgroup1[2],valgroup1[3],valgroup1[4],valgroup1[5],valgroup1[6]
					,valgroup1[7],valgroup1[8],valgroup1[9],valgroup1[10],valgroup1[11],valgroup1[12],valgroup1[13],valgroup1[14]);	 
}

void read_otp_data(struct msm_camera_i2c_client *sensor_i2c_client)
{
	int err;
 	uint32_t addr=0;
       uint16_t val123=0;
	uint16_t valgroup1[7];
	uint16_t valgroup2[7];
	char     otpdata123[30]; 
	int       intvalgroup1[7];
	int       intvalgroup2[7];	
	char    preotpdata[15]="04080-000";
	int IsGroup1;
	int IsGroup2;
	int modulever;
	...
	mdelay(1);
			...
			err=sensor_i2c_client->i2c_func_tbl->i2c_read(sensor_i2c_client,0x700c,&val123,
						MSM_CAMERA_I2C_BYTE_ADDR);

                     printk("====  Basic Flag=0x%x  ====\r\n",val123);
			IsGroup1 = val123 & 0xC0;
			IsGroup2 = val123 & 0x30;
                     printk("====   group1 =0x%x  group2=0x%x ====\r\n",IsGroup1,IsGroup2);			
			if(IsGroup1==0x40)//(val123==0x40)
			{
		         for(addr=0x700d;addr<0x7014;addr++){
					err=sensor_i2c_client->i2c_func_tbl->i2c_read(sensor_i2c_client,addr,&valgroup1[addr-0x700d],
						MSM_CAMERA_I2C_BYTE_ADDR);
					     //printk("======group1[0x%x]=0x%x ======= \r\n",addr-0x700d, valgroup1[addr-0x700d]);
					intvalgroup1[addr-0x700d]=(int)valgroup1[addr-0x700d];
					printk("======intgroup1[0x%x]=%d ======= \r\n",addr-0x700d, intvalgroup1[addr-0x700d]);					
					msleep(2);
		         	}
			  	sprintf(otpdata123,"%s%d%d%02d%06d",preotpdata,intvalgroup1[0]*256+intvalgroup1[1],intvalgroup1[2],intvalgroup1[3],
					intvalgroup1[4]*256*256+intvalgroup1[5]*256+intvalgroup1[6]);
				//printk("%s",otpdata);
				modulever = intvalgroup1[0]*256+intvalgroup1[1];
			} 
				 if(IsGroup2==0x10)//(val123==0x04)
				 	{
				 			for(addr=0x7014;addr<0x701b;addr++){
								err=sensor_i2c_client->i2c_func_tbl->i2c_read(sensor_i2c_client,addr,&valgroup2[addr-0x7014],
								MSM_CAMERA_I2C_BYTE_ADDR);
							printk("----- group2[0x%x]=0x%x ----- \r\n",addr-0x7014, valgroup2[addr-0x7014]);
								intvalgroup2[addr-0x7014]=(int)valgroup2[addr-0x7014];
					              printk("----- intgroup2[0x%x]=%d ----- \r\n",addr-0x7014, intvalgroup2[addr-0x7014]);													
							msleep(2);
		         		}
					sprintf(otpdata123,"%s%d%d%02d%06d",preotpdata,intvalgroup2[0]*256+intvalgroup2[1],intvalgroup2[2],intvalgroup2[3],
						intvalgroup2[4]*256*256+intvalgroup2[5]*256+intvalgroup2[6]);
				       modulever = intvalgroup2[0]*256+intvalgroup2[1];
				 }
				 //modulever = intvalgroup1[0]*256+intvalgroup1[1];
				 if(modulever==56500 ||modulever==56700)
				 	{
				 		printk("<<<<  this is ov5695 reb  >>>>>");
				 		isreb = true;
				 	}else if(modulever==56300 ||modulever==56400){
				 		printk("<<<<  this is ov5695 rea  >>>>>");
				 		isreb = false;
				 	}else
				 	      isreb = true;
					
	sensor_i2c_client->i2c_func_tbl->i2c_write(sensor_i2c_client, 0x0100 , 0x00  ,MSM_CAMERA_I2C_BYTE_ADDR);	

	if(otpdata123!=NULL)
		memcpy(OTPDATA,otpdata123,sizeof(otpdata123));

	printk(" ov5695 unique id = %s ", OTPDATA);
}
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(chipid==0x5695)  //ASUS_BSP : darrency_lin add for read id
		{
	           read_otp_data(sensor_i2c_client);
		    read_allotp_data(sensor_i2c_client);
		}
...
// [5816][Camera][akenhsu] Add use different flash params for different module 20160315 BEGIN
#include <linux/arima_flash.h>
enum camera_mods_5816 e_5816_cam_mod = MOD_NOT_SET;
// [5816][Camera][akenhsu] 20160315 END
int32_t msm_sensor_platform_probe(struct platform_device *pdev, void *data)
{
	...
	//ASUS_BSP Stimber_Hsueh "Implement for ATD interface" +++
		if(strcmp(s_ctrl->sensordata->sensor_name,"ov13850") == 0 ||
			strcmp(s_ctrl->sensordata->sensor_name,"ov8858") == 0)
		{
			create_rear_status_proc_file();
		}else if(strcmp(s_ctrl->sensordata->sensor_name,"s5k5e2_5816") == 0){
			create_front_status_proc_file();
		}else if (strcmp(s_ctrl->sensordata->sensor_name,"ov5695") == 0){
			create_front_status_proc_file();
			create_readotp_proc_file();
			create_printotp_proc_file();
		}
		//ASUS_BSP Stimber_Hsueh "Implement for ATD interface" ---
	...
	if (rc < 0) {
		...
		if(strcmp(s_ctrl->sensordata->sensor_name,"ov13850") == 0 ||
			strcmp(s_ctrl->sensordata->sensor_name,"ov8858") == 0)
		{
			g_camera_status1[g_rear_camera_ref_cnt] = 0;
		}else if(strcmp(s_ctrl->sensordata->sensor_name,"s5k5e2_5816") == 0 ){
...
//module_ID = 0 ~ jvie2
//module_ID != 0 ~~ lens_ID = 0xDE ~ jvie2
//                      ~~ lens_ID = 0x52 ~ jvie(for cts , fix version to jvie2 in 12/23)
if(rc == 0)
{
	if(Main_13M_Camera_version == 0x00)
		{
				camera_actuator_index = 9;
				snprintf(s_ctrl->sensordata->sensor_info->sensor_name,
				sizeof(s_ctrl->sensordata->sensor_info->sensor_name), "%s_jvie2",
				s_ctrl->sensordata->sensor_name);
		}else
			{
				if(Main_Camera_Lens_ID==0xDE) // mp
					{
						camera_actuator_index = 9;
						snprintf(s_ctrl->sensordata->sensor_info->sensor_name,
						sizeof(s_ctrl->sensordata->sensor_info->sensor_name), "%s_jvie2",
						s_ctrl->sensordata->sensor_name);
					}else if(Main_Camera_Lens_ID==0x52) // cts
						{
							camera_actuator_index = 9;
							snprintf(s_ctrl->sensordata->sensor_info->sensor_name,
	...
	if(rc == 0){
		...
		camera_actuator_index = 6;
			snprintf(s_ctrl->sensordata->sensor_info->sensor_name,
				...
				s_ctrl->sensordata->sensor_name);
// [5816][Camera][akenhsu] Add use different flash params for different module 20160315 BEGIN
			e_5816_cam_mod = MAIN_13M;
// [5816][Camera][akenhsu] 20160315 END
		}
		else{
//[5816][Camera]Modify for Judge JVIE different Lens Person Liu 20160330 S
			if(Main_Camera_Lens_ID == 0x33){
...
//[5816][Camera]Modify for Judge JVIE different Lens Person Liu 20160330 E
// [5816][Camera][akenhsu] Add use different flash params for different module 20160315 BEGIN
			e_5816_cam_mod = JVIE_13M;
// [5816][Camera][akenhsu] 20160315 END
		}
...
// [5816][Camera][akenhsu] 20151211 END
	...
	//[ZB501KL]ASUS BSP Ryan +++ for ov5695 sensor name setting
	if(isreb==true)
{
	rc = strcmp(s_ctrl->sensordata->sensor_name,"ov5695");
	if(rc == 0){
			snprintf(s_ctrl->sensordata->sensor_info->sensor_name,
				sizeof(s_ctrl->sensordata->sensor_info->sensor_name),"%s_reb",
				s_ctrl->sensordata->sensor_name);
			// if front camera jave second source , need to modified here to check different sensor name
		}
}else
{
	rc = strcmp(s_ctrl->sensordata->sensor_name,"ov5695");
...
CDBG(">>>>>> %s: %s probe succeeded\n", __func__,
	...
	INIT_WORK(&s_ctrl->work, sensor_powerdown_work); //ASUS_BSP: Darrency_lin -- decrease switch time
...
//--- darrency_lin ++
#define  OTPDATA_READ_FILE              "driver/readotp"
static struct proc_dir_entry *readotp_proc_file;

static int   readotp_proc_read(struct seq_file *buf, void *v)
{
	printk(" %s -- otpdata=%s \r\n",__func__,OTPDATA);
	if(OTPDATA!=NULL)
			seq_printf(buf, "%s\n", OTPDATA);
...
static int readotp_proc_open(struct inode *inode, struct  file *file)
{
    return single_open(file, readotp_proc_read, NULL);

}
static const struct file_operations readotp_status_fops = {
	.owner = THIS_MODULE,
	.open = readotp_proc_open,
	.read = seq_read,
...
static void create_readotp_proc_file(void)
{
	        readotp_proc_file = proc_create(OTPDATA_READ_FILE, 0666, NULL, &readotp_status_fops);
		if(readotp_proc_file) {
			CDBG("%s sucessed!\n", __func__);
		} else {
			pr_err("%s failed!\n", __func__);
		}
}
#define  OTPDATA_PRINT_FILE              "driver/printotp"
static struct proc_dir_entry *printotp_proc_file;

static int   printotp_proc_read(struct seq_file *buf, void *v)
{
	if(ALLOTPDATA!=NULL)
			seq_printf(buf, "%s\n", ALLOTPDATA);
	else
...
static int printotp_proc_open(struct inode *inode, struct  file *file)
{
    return single_open(file, printotp_proc_read, NULL);

}
static const struct file_operations printotp_status_fops = {
	.owner = THIS_MODULE,
	.open = printotp_proc_open,
	.read = seq_read,
	//.write = status_proc_write,
	.llseek = seq_lseek,
	.release = single_release,
...
static void create_printotp_proc_file(void)
{
	        printotp_proc_file = proc_create(OTPDATA_PRINT_FILE, 0666, NULL, &printotp_status_fops);
		if(printotp_proc_file) {
			CDBG("%s sucessed!\n", __func__);
		} else {
			pr_err("%s failed!\n", __func__);
		}
...
static void remove_proc_file(void)
{
	...
	memset(g_camera_status1, 0, sizeof(g_camera_status1));
