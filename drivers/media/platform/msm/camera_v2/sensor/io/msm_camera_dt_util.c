...
extern int Main_13M_Camera_version;  //ASUS_BSP : darrency_lin add 

extern void msm_actuator_close_move(void);//ASUS_BSP: Darrency_lin ++ decrease switch time
				    ...
				    struct  msm_sensor_info_t **s_info)
{
	...
	rc = of_property_read_u32(of_node, "qcom,sensor-position",&sensor_pos);
//printk(" sensor pos=%d",sensor_pos);
printk("<<< check ov13850 eeprom exist  camera_version=0x%x >>>", Main_13M_Camera_version);
if(Main_13M_Camera_version||sensor_pos)  //read unique id first, if failure dont add eeprom to subdev
{
...
if(sensor_i2c_client->sensorname!=NULL){
	if((!strcmp(sensor_i2c_client->sensorname,"ov13850"))||(!strcmp(sensor_i2c_client->sensorname,"ov13850_jvie"))||(!strcmp(sensor_i2c_client->sensorname,"ov13850_jvie2")))
		msm_actuator_close_move();
}
//ASUS_BSP: Darrency_lin -- decrease switch time
