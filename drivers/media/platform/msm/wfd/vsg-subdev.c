...
unsigned long g_counter_skip_encode = 0;
long jiffies_last = 0, jiffies_cur = 0;
...
static void vsg_work_func(struct work_struct *task)
{
	...
	list_for_each_entry(temp, &context->busy_queue.node, node) {
		if (++count > MAX_BUFS_BUSY_WITH_ENC) {
			g_counter_skip_encode++;
			jiffies_cur = jiffies;
			if(jiffies_to_msecs(abs(jiffies_cur-jiffies_last)) > 1000){
				WFD_MSG_WARN(
					"Skipping encode, too many buffers with encoder, count=%lu\n", g_counter_skip_encode);
