...
#if !defined(CONFIG_ENABLE_CORESIGHT_CLK)
...
#if defined(CONFIG_ENABLE_CORESIGHT_CLK)
#if defined(CONFIG_MSM_JTAG_MM)
    #error "You dont' have to config CONFIG_ENABLE_CORESIGHT_CLK with CONFIG_MSM_JTAG_MM"
#else
    if (!is_device_fused()){
        printk("ASUS:None Fused device detected! Enable coresight clk to prevent TZ crashed device");

        clock[0] = devm_clk_get(dev, "core_clk");
        if (!IS_ERR(clock[0])) {
            if (!clk_set_rate(clock[0], CORESIGHT_CLK_RATE_TRACE))
                clk_prepare_enable(clock[0]);
        }
    }
    return -EPERM;
#endif
