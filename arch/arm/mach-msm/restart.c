...
int g_uart_enable = 0;
static int __init get_enable_uart_status(char *str)
{
	if(strcmp("y", str) == 0){
		g_uart_enable = 1;
		printk("restart: uart has been enabled\n");
	}else{
		printk("restart: uart has been disabled\n");
	}
	return 0;
}
__setup("bootdbguart=", get_enable_uart_status);
...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (cmd != NULL) {
		if (!strncmp(cmd, "bootloader", 10)) {
			...
			// reboot reason = adb reboot oem-88
...
static int __init msm_restart_init(void)
{
    ...
    if(g_uart_enable){
        printk("restart: enable QPST\n");
        download_mode = 1;
    }
    else{
        printk("restart: disable QPST\n");
        download_mode = 0;
	}
#endif
	set_dload_mode(download_mode);
