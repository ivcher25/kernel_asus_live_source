...
static struct wake_lock wlock; //AllenCH_Lin +++
...
irqreturn_t wcd9xxx_dce_handler(int irq, void *data)
{
	...
	wake_lock_timeout(&wlock, 0.5*HZ); //AllenCH_Lin +++
		      ...
		      bool impedance_det_en)
{
	...
	wake_lock_init(&wlock, WAKE_LOCK_SUSPEND,"wlock"); //AllenCH_Lin +++
...
void wcd9xxx_mbhc_deinit(struct wcd9xxx_mbhc *mbhc)
{
	...
	wake_lock_destroy(&wlock); //AllenCH_Lin +++
	mutex_destroy(&mbhc->mbhc_lock);
	wcd9xxx_resmgr_unregister_notifier(mbhc->resmgr, &mbhc->nblock);
