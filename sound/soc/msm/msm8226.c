...
static struct request_gpio pri_mi2s_gpio[] = {
        ...
        {
                .gpio_name = "PRI_MI2S_SD0",
        },

 /*       {
                .gpio_name = "PRI_MI2S_SD1",
        },*/
};
...
static int msm8226_dtparse_mi2s(void)
{
        pri_mi2s_gpio[0].gpio_no = of_get_named_gpio(spdev->dev.of_node,
                "qcom,prim-mi2s-gpio-sclk", 0);
        if (pri_mi2s_gpio[0].gpio_no < 0) {
                pr_err("%s: MI2S_SCLK GPIO error in the device tree\n",
                        __func__);
                return -EINVAL;
        }
        pri_mi2s_gpio[1].gpio_no = of_get_named_gpio(spdev->dev.of_node,
                "qcom,prim-mi2s-gpio-ws", 0);
        ...
        pri_mi2s_gpio[2].gpio_no = of_get_named_gpio(spdev->dev.of_node,
                "qcom,prim-mi2s-gpio-data0", 0);
        if (pri_mi2s_gpio[2].gpio_no < 0) {
                pr_err("%s: MI2S_SD0 GPIO error in the device tree\n",
                        __func__);
                return -EINVAL;
        }

/*        pri_mi2s_gpio[3].gpio_no = of_get_named_gpio(spdev->dev.of_node,
                "qcom,prim-mi2s-gpio-data1", 0);
        if (pri_mi2s_gpio[3].gpio_no < 0) {
                pr_err("%s: MI2S_SD1 GPIO error in the device tree\n",
                        __func__);
                return -EINVAL;
        }
*/
...
#undef S
	 ...
	 * 0-70    == Button 0
	 * 110-180 == Button 1
	 * 210-290 == Button 2 vol+
	 * 360-680 == Button 3 vol-
	 */
	btn_low[0] = 0;
	btn_high[0] = 70;
	btn_low[1] = 110;
...
/*		ret = snd_soc_dai_set_fmt(codec_dai, SND_SOC_DAIFMT_CBS_CFS);
    ...
    printk("msm8226_pri_mi2s_startup mi2s start ");
    return ret;
}
