...
int gSkypeState = 0;  //AllenCH_Lin +++
static int voice_call_state = -1;  //AllenCH_Lin +++
static int phone_state = -1;  //AllenCH_Lin +++
...
//AllenCH_Lin +++
	...
	case AUDIO_GET_SKYPE_STATE:
		...
		if (copy_from_user(&voice_call_state, (void *)arg,
				sizeof(voice_call_state))) {
			pr_err("%s: Could not copy voice_call_state from user\n", __func__);
			result = -EFAULT;
		}
